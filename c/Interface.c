    ...
    // 區分操作系統 Windows MingW-w64 gcc 或 Linux-Ubuntu ( Android-Termux ) gcc 系統;
        ...
        if (strncmp(IPversion, "IPv4", sizeof(IPversion)) == 0) {
            ...
            while (1) {
                ...
                memset(message_returned_from_server_when_Handshake_successful, 0x00, sizeof(message_returned_from_server_when_Handshake_successful));
                strncpy(message_returned_from_server_when_Handshake_successful, "Standby ...", sizeof(message_returned_from_server_when_Handshake_successful) - 1); // 確保留出空間給終止字符：'\0';
                message_returned_from_server_when_Handshake_successful[sizeof(message_returned_from_server_when_Handshake_successful) - 1] = '\0'; // 確保終止字符：'\0' 存在;
                send(ClientSocket, message_returned_from_server_when_Handshake_successful, strlen(message_returned_from_server_when_Handshake_successful), 0);
                ...
                while (1) {
                    ...
                    // printf("%s\n", response_Data);
                    // 判斷從標準輸入設備（stdin）傳入信息是否爲字符串「"exit"」，如是則中止運行;
                    if (strncmp(response_Data, "exit", sizeof(response_Data)) == 0) {
                        printf("Standard input (stdio) : [ %s ] received.\nProgram aborted.\n", response_Data);
                        // closesocket(ClientSocket);
                        memset(RecvBuffer, 0x00, sizeof(RecvBuffer));
                        // memset(response_Data, 0x00, sizeof(response_Data));
                        memset(requestConnection, 0x00, sizeof(requestConnection));
                        // 將 "Connection:close" 值信號傳遞給當前 while 循環，用以終止跳出當前 while 循環;
                        strncpy(requestConnection, "close", sizeof(requestConnection) - 1); // 確保留出空間給終止字符：'\0';
                        requestConnection[sizeof(requestConnection) - 1] = '\0'; // 確保終止字符：'\0' 存在;
                        // 將 "exit" 值信號傳遞給外部 while 循環，用以終止跳出外部 while 循環;
                        strncpy(RecvBuffer, "exit", sizeof(RecvBuffer) - 1); // 確保留出空間給終止字符：'\0';
                        RecvBuffer[sizeof(RecvBuffer) - 1] = '\0'; // 確保終止字符：'\0' 存在;
                        // break;
                        // return 2;
                        // exit(signum);
                        // exit(2);
                    }
                    // 判斷從標準輸入設備（stdin）傳入信息是否爲字符串「"Connection:close"」，如是則中止運行;
                    if (strncmp(response_Data, "Connection:close", sizeof(response_Data)) == 0) {
        ...
        if (strncmp(IPversion, "IPv4", sizeof(IPversion)) == 0) {
            ...
            while (1) {
                ...
                // // 創建一個子進程（thread），接收用戶端（Client）發送的請求（request）數據，并計算處理，然後將處理結果返回給用戶端;
                // hThread = CreateThread(NULL, 0, ClientThread, (LPVOID)sockfdClient, 0, NULL);
                // if (hThread == NULL) {
                //     printf("Create thread failed.\n");
                //     break;
                // }
                ...
                while (1) {
                    ...
                    // temp[len] = '\0'; // 確保 temp 是正確終結的字符串;
                    // char *token = strtok(response_Data, "\n");  // 函數：strtok() 表示分割字符串，本例使用換行符號（\n）分割字符串，即刪除原字符串末尾的換行符號（\n）;
                    // printf("%s\n", response_Data);// 判斷信號值（signum）是否爲「Ctrl+c」;
                    // 判斷從標準輸入設備（stdin）傳入信息是否爲字符串「"exit"」，如是則中止運行;
                    if (strncmp(response_Data, "exit", sizeof(response_Data)) == 0) {
                        printf("Standard input (stdio) : [ %s ] received.\nProgram aborted.\n", response_Data);
                        // close(sockfdClient);
                    ...
                    Ret = 0;
                    // Ret = send(sockfdClient, SendBuffer, (int)strlen(SendBuffer), 0);
                    Ret = send(sockfdClient, response_Data, strlen(response_Data), 0);
                    if (Ret < 0) {
                        printf("Send information error.\n");
                        close(sockfdClient);
        ...
        if (strncmp(IPversion, "IPv4", sizeof(IPversion)) == 0) {
            ...
            while (1) {
                ...
                if (str_len < 0) {
                    ...
                    if (strncmp(RecvBuffer, "exit", sizeof(RecvBuffer)) == 0 || strncmp(RecvBuffer, "Connection:close", sizeof(RecvBuffer)) == 0) {
