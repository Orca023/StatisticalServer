Attribute VB_Name = "AlgorithmDispatchModule"
...
Sub MenuSetup()
    ...
    '插入統計計算 menuStatistics 操作菜單
    '判斷是否自定義的菜單已存在，菜單不允許重名，如果已存在，則先刪除
    'Application.CommandBars("Worksheet Menu Bar").Controls("統計運算 Statistics").Delete
    Dim ctl As CommandBarControl
    For Each ctl In Application.CommandBars("Worksheet Menu Bar").Controls
        If ctl.Caption = "統計運算 Statistics" Then ctl.Delete
    Next ctl

    Dim menuStatistics As CommandBarPopup  '頂層菜單，CommandBarPopup 表示下拉菜單類型
    Set menuStatistics = menuBar.Controls.Add(Type:=msoControlPopup, Temporary:=True)  '參數 Type:=msoControlPopup 表示下拉菜單類型；參數 Temporary:=True 表示臨時新增菜單，Excel 關閉後會自動刪除。
    menuStatistics.Caption = "統計運算 Statistics"  '自定義的頂層菜單名
    menuStatistics.TooltipText = "統計糢型計算"  '菜單提示文字

    Dim menuStatisticsServer As CommandBarButton  '一級子菜單，CommandBarButton 表示按鈕類型。 'CommandBarPopup  '一級子菜單，CommandBarPopup 表示下拉菜單類型
    Set menuStatisticsServer = menuStatistics.Controls.Add(Type:=msoControlButton)  '參數 Type:=msoControlButton 表示按鈕類型。
    menuStatisticsServer.Caption = "統計運算伺服器 Statistics server"  '自定義的一級子菜單名
    'menuStatisticsServer.Caption = "統計運算 Statistics server(&S)"  '自定義的一級子菜單名，參數 (&S) 表示配置的啓動快捷鍵，括號中的 &S 表示快捷鍵：Ctrl + Shift + S
    menuStatisticsServer.TooltipText = "對應不同的統計模型，提供相應的計算方法的伺服器"  '菜單提示文字
    menuStatisticsServer.Style = msoButtonIconAndCaption  '菜單樣式（圖標加文字）
    menuStatisticsServer.FaceId = 263  '圖標代號
    'menuStatisticsServer.ShortcutText = "Ctrl + Shift + S"  '提示快捷鍵的顯示字符串：Ctrl + Shift + S
    menuStatisticsServer.OnAction = "runStatisticsAlgorithmServer"  '左鍵單擊後執行的宏名稱字符串
    menuStatisticsServer.BeginGroup = True  '添加分割綫



    Dim menuStatisticsPanel As CommandBarButton  '一級子菜單，CommandBarButton 表示按鈕類型
    Set menuStatisticsPanel = menuStatistics.Controls.Add(Type:=msoControlButton)  '參數 Type:=msoControlButton 表示按鈕類型。
    menuStatisticsPanel.Caption = "人機交互介面 operation panel" '自定義的一級子菜單名
    'menuStatisticsPanel.Caption = "人機交互介面 operation panel(&S)" '自定義的一級子菜單名，參數 (&S) 表示配置的啓動快捷鍵，括號中的 &S 表示快捷鍵：Ctrl + Shift + S
    menuStatisticsPanel.TooltipText = "統計運算的人機交互介面操作面板"  '菜单提示文字
    menuStatisticsPanel.Style = msoButtonIconAndCaption  '菜單樣式（圖標加文字）
    menuStatisticsPanel.FaceId = 263  '圖標代號
    'menuStatisticsPanel.ShortcutText = "Ctrl + Shift + S"  '提示快捷鍵的顯示字符串：Ctrl + Shift + S
    menuStatisticsPanel.OnAction = "AlgorithmDispatchModule.StatisticsAlgorithm"  '左鍵單擊後執行的宏名稱字符串
    menuStatisticsPanel.BeginGroup = True  '添加分割綫


    'Dim menuStatisticsPanel As CommandBarPopup  '一級子菜單，CommandBarPopup 表示下拉菜單類型
    'Set menuStatisticsPanel = menuStatistics.Controls.Add(Type:=msoControlPopup)  '參數 Type:=msoControlPopup 表示下拉菜單類型。
    'menuStatisticsPanel.Caption = "人機交互介面 operation panel"  '自定義的一級子菜單名
    'menuStatisticsPanel.TooltipText = "統計運算的人機交互介面操作面板"  '菜單提示文字

    'Dim menuStatisticsPanelTest As CommandBarButton  '二級子菜單，CommandBarButton 表示按鈕類型
    'Set menuStatisticsPanelTest = menuStatisticsPanel.Controls.Add(Type:=msoControlButton)  '參數 Type:=msoControlButton 表示按鈕類型。
    'menuStatisticsPanelTest.Caption = "示例工程 test" '自定義的二級子菜單名
    ''menuStatisticsPanelTest.Caption = "示例工程 test(&T)" '自定義的二級子菜單名，參數 (&T) 表示配置的啓動快捷鍵，括號中的 &T 表示快捷鍵：Ctrl + Shift + T
    'menuStatisticsPanelTest.TooltipText = "統計運算示例工程 test 的人機交互介面操作面板"  '菜单提示文字
    'menuStatisticsPanelTest.Style = msoButtonIconAndCaption  '菜單樣式（圖標加文字）
    'menuStatisticsPanelTest.FaceId = 263  '圖標代號
    ''menuStatisticsPanelTest.ShortcutText = "Ctrl + Shift + T"  '提示快捷鍵的顯示字符串：Ctrl + Shift + T
    'menuStatisticsPanelTest.OnAction = "testStatisticsAlgorithm"  '左鍵單擊後執行的宏名稱字符串
    'menuStatisticsPanelTest.BeginGroup = True  '添加分割綫

End Sub


Sub runStatisticsAlgorithmServer()

    '聲明變量
    Dim codeScript As String, shellPIDvalue As Integer

    '檢查是否已經開啓過服務器，如果已經開啓則退出當前過程，如果未開啓過則執行開啓服務器
    If shellPIDdict.Exists("StatisticsServer") Then

        Dim oWMT As Object, oProcess As Object
        Set oWMT = GetObject("winmgmts:")
        For Each oProcess In oWMT.InstancesOf("Win32_Process")
            If oProcess.ProcessID = shellPIDdict.Item("StatisticsServer") Then
                Debug.Print "running Statistics server PID: " & oProcess.ProcessID
                'Application.CommandBars("Worksheet Menu Bar").Controls("統計運算 Statistics").Controls("統計運算 Statistics server").Caption = "統計運算 Statistics server(running)"   '變更顯示文字
                ''Application.CommandBars("Worksheet Menu Bar").Controls("統計運算 Statistics").Controls("統計運算 Statistics server").Font.ColorIndex = 15   '變更顯示字體
                ''Application.CommandBars("Worksheet Menu Bar").Controls("統計運算 Statistics").Controls("統計運算 Statistics server").Enabled = False   '禁用
                Exit Sub
            End If
        Next
        Set oWMT = Nothing
        ...
        'codeScript = CStr(CStr(ThisWorkbook.Path) & "\StatisticalServerPython\Scripts\python.exe" & " " & CStr(ThisWorkbook.Path) & "\StatisticalServerPython\src\StatisticalAlgorithmServer.py")  '開啓服務器的命令行控制臺 cmd 命令全名，函數：CStr(ThisWorkbook.Path) 表示 Excel 文檔（.xlsm）所在的路徑;
        'Debug.Print codeScript
        'codeScript = "C:\StatisticalServer\Julia\Julia-1.10.10\bin\julia.exe -p 4 --project=C:/StatisticalServer/StatisticalServerJulia/ C:/StatisticalServer/StatisticalServerJulia/src/StatisticalAlgorithmServer.jl"  '開啓服務器的命令行控制臺 cmd 命令全名
        codeScript = "C:\StatisticalServer\Julia\Julia-1.10.10\bin\julia.exe --project=C:/StatisticalServer/StatisticalServerJulia/ C:/StatisticalServer/StatisticalServerJulia/src/StatisticalAlgorithmServer.jl"  '開啓服務器的命令行控制臺 cmd 命令全名
        'codeScript = "C:\StatisticalServer\StatisticalServerPython\Scripts\python.exe C:/StatisticalServer/StatisticalServerPython/src/StatisticalAlgorithmServer.py"  '開啓服務器的命令行控制臺 cmd 命令全名
        'codeScript = "C:\StatisticalServer\Python\Python311\python.exe C:/StatisticalServer/StatisticalServerPython/src/StatisticalAlgorithmServer.py"  '開啓服務器的命令行控制臺 cmd 命令全名
        ...
        Debug.Print "running Statistics server PID: " & shellPIDvalue
        'Application.CommandBars("Worksheet Menu Bar").Controls("統計運算 Statistics").Controls("統計運算 Statistics server").Caption = "統計運算 Statistics server(running)"   '變更顯示文字
        ''Application.CommandBars("Worksheet Menu Bar").Controls("統計運算 Statistics").Controls("統計運算 Statistics server").Font.ColorIndex = 15   '變更顯示字體
        ''Application.CommandBars("Worksheet Menu Bar").Controls("統計運算 Statistics").Controls("統計運算 Statistics server").Enabled = False   '禁用
        ...
        If shellPIDdict.Exists("StatisticsServer") Then
            shellPIDdict.Item("StatisticsServer") = shellPIDvalue
        Else
            shellPIDdict.Add "StatisticsServer", shellPIDvalue
        End If
    ...
    End If
    'Debug.Print shellPIDdict.Item("StatisticsServer")

End Sub

'Sub testStatisticsAlgorithm()
'
...
'    moduleName = "testStatisticsAlgorithmModule"  '統計算法模塊的自定義命名
'    modulePath = "C:\Criss\vba\Statistics\StatisticsAlgorithmServer\test"  '統計算法模塊的保存路徑
'    moduleFileName = "testStatisticsAlgorithmModule.bas"  '統計算法模塊的文檔名
...
'        End If
'
'        '執行導入模塊 testStatisticsAlgorithmModule 中的自定義子過程 Sub testStatisticsAlgorithm()，函數 .Evaluate () 表示將字符串解析為代碼並執行
'        'Application.Run "testStatisticsAlgorithmModule.testStatisticsAlgorithm"
'        Application.Run moduleName & "." & Split(moduleName, "Module")(0)
'        'Call testStatisticsAlgorithmModule.testStatisticsAlgorithm
'        'Application.Evaluate ("Call testStatisticsAlgorithmModule.testStatisticsAlgorithm()")  '執行導入模塊中的自定義子過程 testStatisticsAlgorithm，函數 .Evaluate () 表示將字符串解析為代碼並執行
'        'Application.Evaluate ("Call " & moduleName & "." & Split(moduleName, "Module")(0) & "()")  '執行導入模塊中的自定義子過程 testStatisticsAlgorithm，函數 .Evaluate () 表示將字符串解析為代碼並執行
'
'        ''判斷使用的爬蟲模塊名稱，可以取值：("testStatisticsAlgorithmModule", "LC5PStatisticsAlgorithmModule")
'        'Select Case moduleName
'
'        '    Case "testStatisticsAlgorithmModule"
'
'        '        Call testStatisticsAlgorithmModule.testStatisticsAlgorithm  '執行導入模塊中的自定義子過程 testStatisticsAlgorithm，函數
'        '        'ThisWorkbook.VBProject.VBComponents("testStatisticsAlgorithmModule").Controls("testStatisticsAlgorithm")
'
'        '        'Exit Sub
'
'        '    Case "InterpolationStatisticsAlgorithmModule"
'
'        '    Case "Polynomial3StatisticsAlgorithmModule"
'
'        '    Case "LC5PStatisticsAlgorithmModule"
'
'        '        'Call LC5PStatisticsAlgorithmModule.LC5PStatisticsAlgorithm  '執行導入模塊中的自定義子過程 LC5PStatisticsAlgorithm，函數
'        '        ''ThisWorkbook.VBProject.VBComponents("LC5PStatisticsAlgorithmModule").Controls("LC5PStatisticsAlgorithm")
'
'        '        'Exit Sub
'
'        '    Case Else
...
'        'Application.Evaluate ("Call " & moduleName & "." & Split(moduleName, "Module")(0) & "()")  '執行導入模塊中的自定義子過程 testStatisticsAlgorithm，函數 .Evaluate () 表示將字符串解析為代碼並執行
'
'        ''判斷使用的統計算法模塊名稱，可以取值：("testStatisticsAlgorithmModule", "LC5PStatisticsAlgorithmModule")
'        'Select Case moduleName
'
'        '    Case "testStatisticsAlgorithmModule"
'
...
Sub StatisticsAlgorithm()
    ...
    '自定義的待導入的統計算法模塊
    Dim moduleName As String, modulePath As String, moduleFileName As String, moduleFile As String
    moduleName = "StatisticsAlgorithmModule"  '統計算法模塊的自定義命名
    modulePath = CStr(ThisWorkbook.Path)  'Excel 文檔（.xlsm）所在的路徑，統計算法模塊的保存路徑
    'modulePath = "C:\Criss\vba\Statistics\StatisticsAlgorithmServer"  '統計算法模塊的保存路徑
    moduleFileName = "StatisticsAlgorithmModule.bas"  '統計算法模塊的文檔名
    moduleFile = modulePath & "\" & moduleFileName  '統計算法模塊文檔的路徑全名
    ...
    If ThisWorkbook.VBProject.VBComponents(moduleName) Is Nothing Then
        ...
        '執行導入模塊 StatisticsAlgorithmModule 中的自定義子過程 Sub StatisticsAlgorithm()，函數 .Evaluate () 表示將字符串解析為代碼並執行
        'Application.Run "StatisticsAlgorithmModule.StatisticsAlgorithm"
        Application.Run moduleName & "." & Split(moduleName, "Module")(0)
        'Call StatisticsAlgorithmModule.StatisticsAlgorithm
        'Application.Evaluate ("Call StatisticsAlgorithmModule.StatisticsAlgorithm()")  '執行導入模塊中的自定義子過程 StatisticsAlgorithm，函數 .Evaluate () 表示將字符串解析為代碼並執行
        'Application.Evaluate ("Call " & moduleName & "." & Split(moduleName, "Module")(0) & "()")  '執行導入模塊中的自定義子過程 StatisticsAlgorithm，函數 .Evaluate () 表示將字符串解析為代碼並執行

        ''判斷使用的統計算法模塊名稱，可以取值：("StatisticsAlgorithmModule")
        'Call StatisticsAlgorithmModule.StatisticsAlgorithm  '執行導入模塊中的自定義子過程 StatisticsAlgorithm，函數
        ''ThisWorkbook.VBProject.VBComponents("StatisticsAlgorithmModule").Controls("StatisticsAlgorithm")
        ''Exit Sub
