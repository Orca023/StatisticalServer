...
input_K_Line_Daily_Data_String = ""  # 初始日棒缐（K-Line）初始數據字典字符串，從控制臺傳入值;
                ...
                for line in lines:
                    ...
                    if isinstance(line, str) and line != "":
                        ...
                        # 初始日棒缐（K-Line）初始數據字典字符串，從控制臺傳入值 "{"date_transaction" : [str(datetime.date("2024-08-09"))], "turnover_volume" : [int()], "opening_price" : [float()], "close_price" : [float()], "low_price" : [float()], "high_price" : [float()]}" ;
                            ...
                            input_K_Line_Daily_Data_String = str(line_Value)  # 初始日棒缐（K-Line）初始數據字典字符串，從控制臺傳入值 "{"date_transaction" : [str(datetime.date("2024-08-09"))], "turnover_volume" : [int()], "opening_price" : [float()], "close_price" : [float()], "low_price" : [float()], "high_price" : [float()]}" ;
                            # print("input K-Line source JSON string :\n", input_K_Line_Daily_Data_String)
                            continue
                        else:
        ...
        if i > 0:
            ...
            if isinstance(sys.argv[i], str) and sys.argv[i] != "" and sys.argv[i].find("=", 0, int(len(sys.argv[i])-1)) != -1:
                ...
                # 初始日棒缐（K-Line）初始數據字典字符串，從控制臺傳入值 "{"date_transaction" : [str(datetime.date("2024-08-09"))], "turnover_volume" : [int()], "opening_price" : [float()], "close_price" : [float()], "low_price" : [float()], "high_price" : [float()]}" ;
                    ...
                    input_K_Line_Daily_Data_String = str(sys.argv[i].split("=", -1)[1])  # 初始日棒缐（K-Line）初始數據字典字符串，從控制臺傳入值 "{"date_transaction" : [str(datetime.date("2024-08-09"))], "turnover_volume" : [int()], "opening_price" : [float()], "close_price" : [float()], "low_price" : [float()], "high_price" : [float()]}" ;
                    # print("input K-Line source JSON string :\n", input_K_Line_Daily_Data_String)
                    continue
                else:
                    # print(sys.argv[i], "unrecognized.")
...
if input_K_Line_Daily_Data_String != "":
    input_K_Line_Daily_Data = json.loads(input_K_Line_Daily_Data_String)  # 使用 Python 原生 JSON 模組中的 json.loads() 函數將 JSON 字符串轉換爲 Python 字典（dict）對象;
...
sys.stdout.write(str(json.dumps(stepping_data)))  # 使用 Python 原生 JSON 模組中的 json.dumps() 函數將 Python 字典（dict）對象轉換爲 JSON 字符串;
