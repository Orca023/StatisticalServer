...
class http_Server:
    ...
    def Server(self, host, port, Is_multi_thread, Key, Session, do_Function, number_Worker_process, process_Pool, pool_func, pool_call_back, error_pool_call_back, total_worker_called_number):
        ...
        # 指定單缐程（1-thread）執行並支持 IPv6 地址監聽，注意，使用：xlwings 擴展模組與電子表格（Excel）交互時，使用多缐程會報錯;
        class ThreadedHTTPServer(HTTPServer):
            address_family = socket.AF_INET6  # 配置服務器支持監聽 IPv6 版 IP 地址;
            pass
        # if (Is_multi_thread == True) and (number_Worker_process > 0):
        ...
        #     #     ForkingMixIn.daemon_threads = True
        #     #     address_family = socket.AF_INET6  # 配置服務器支持監聽 IPv6 版 IP 地址;
        #     #     pass
        ...
        #         address_family = socket.AF_INET6  # 配置服務器支持監聽 IPv6 版 IP 地址;
        #         pass
        # elif (Is_multi_thread == False) or (number_Worker_process <= 0):
        #     # 指定單缐程（1-thread）執行並支持 IPv6 地址監聽;
        #     class ThreadedHTTPServer(HTTPServer):
        #         address_family = socket.AF_INET6  # 配置服務器支持監聽 IPv6 版 IP 地址;
        #         pass
        # else:
