...
input_K_Line_Daily_Dict_String::Core.String = Base.string("");  # 初始日棒缐（K-Line）初始數據字典字符串，從控制臺傳入值;
                    ...
                    for line in eachline(config_file_RIO)
                        ...
                        if Core.isa(line, Core.String) && line !== ""
                            ...
                            if line_Key === "K_Line_source"
                                global input_K_Line_Daily_Dict_String = line_Value;  # 初始日棒缐（K-Line）初始數據字典字符串，從控制臺傳入值 "{"date_transaction" => [Base.string(Dates.Date("2024-08-09"))], "turnover_volume" => [Core.Int64()], "opening_price" => [Core.Float64()], "close_price" => [Core.Float64()], "low_price" => [Core.Float64()], "high_price" => [Core.Float64()]}" ;
                                # print("input K-Line source JSON string : ", input_K_Line_Daily_Dict_String, "\n");
                                continue;
                            end
                        end
                    end
            ...
            if Base.length(ARGSIArray) > 1
                ...
                if ARGSValue !== ""
                    ...
                    if ARGSIArray[1] === "K_Line_source"
                        global input_K_Line_Daily_Dict_String = ARGSValue;  # 初始日棒缐（K-Line）初始數據字典字符串，從控制臺傳入值 "{"date_transaction" => [Base.string(Dates.Date("2024-08-09"))], "turnover_volume" => [Core.Int64()], "opening_price" => [Core.Float64()], "close_price" => [Core.Float64()], "low_price" => [Core.Float64()], "high_price" => [Core.Float64()]}" ;
                        # print("input K-Line source JSON string : ", input_K_Line_Daily_Dict_String, "\n");
                        continue;
                    end
                end
...
if Core.isa(input_K_Line_Daily_Dict_String, Core.String) && (Base.length(input_K_Line_Daily_Dict_String) > 0)
    input_K_Line_Daily_Dict = JSONparse(input_K_Line_Daily_Dict_String);  # 使用藉助第三方擴展包「JSON」裏的 JSON.parse() 函數創建的自定義的 JSONparse() 函數將 JSON 字符串轉換爲 Julia 字典（Dict）對象;
end
...
# # 將 Julia 數據類型 Core.Nothing 和 Base.Missing 和 Core.undef 值轉換爲 "null" 字符串類型;
stepping_data_2 = Base.Dict{Core.String, Core.Any}();
if Base.isa(stepping_data, Base.Dict)
    for (key_1, value_1) in stepping_data
        # println("Key_1: $key_1, Value_1:\n$value_1");
        if Base.isa(value_1, Base.Dict)
            stepping_data_2[key_1] = Base.Dict{Core.String, Core.Any}();
            for (key_2, value_2) in value_1
                # println("Key_2: $key_2, Value_2:\n$value_2");
                if (Base.typeof(value_2) <: Core.Array) || (Base.typeof(value_2) <: Base.Vector)
                    stepping_data_2[key_1][key_2] = Core.Array{Core.Any, 1}();
                    for i = 1:Base.length(value_2)
                        if (!(Base.isa(value_2[i], Base.Dict) || (Base.typeof(value_2[i]) <: Core.Array || Base.typeof(value_2[i]) <: Base.Vector) || Base.isa(value_2[i], Core.String))) && (Base.ismissing(value_2[i]) || Base.isnothing(value_2[i]))
                            # stepping_data_2[key_1][key_2][i] = Base.string("null");
                            Base.push!(stepping_data_2[key_1][key_2], Base.string("null"));  # 使用 push! 函數在數組末尾追加推入新元;
                        elseif (!(Base.isa(value_2[i], Base.Dict) || (Base.typeof(value_2[i]) <: Core.Array || Base.typeof(value_2[i]) <: Base.Vector) || Base.isa(value_2[i], Core.String))) && ((Base.typeof(value_2[i]) <: Core.Real) && Base.isinf(value_2[i]) && (value_2[i] > 0.0))
                            # stepping_data_2[key_1][key_2][i] = Base.string("+Infinity");
                            Base.push!(stepping_data_2[key_1][key_2], Base.string("+Infinity"));
                        elseif (!(Base.isa(value_2[i], Base.Dict) || (Base.typeof(value_2[i]) <: Core.Array || Base.typeof(value_2[i]) <: Base.Vector) || Base.isa(value_2[i], Core.String))) && ((Base.typeof(value_2[i]) <: Core.Real) && Base.isinf(value_2[i]) && (value_2[i] < 0.0))
                            # stepping_data_2[key_1][key_2][i] = Base.string("-Infinity");
                            Base.push!(stepping_data_2[key_1][key_2], Base.string("-Infinity"));
                        elseif (!(Base.isa(value_2[i], Base.Dict) || (Base.typeof(value_2[i]) <: Core.Array || Base.typeof(value_2[i]) <: Base.Vector) || Base.isa(value_2[i], Core.String))) && ((Base.typeof(value_2[i]) <: Dates.Date) || (Base.typeof(value_2[i]) <: Dates.DateTime) || (Base.typeof(value_2[i]) <: Dates.Time))
                            # stepping_data_2[key_1][key_2][i] = Base.string(Dates.format(value_2[i], "yyyy-mm-dd"));
                            Base.push!(stepping_data_2[key_1][key_2], Base.string(Dates.format(value_2[i], "yyyy-mm-dd")));
                        else
                            Base.push!(stepping_data_2[key_1][key_2], value_2[i]);
                        end
                    end
                elseif (!(Base.isa(value_2, Base.Dict) || (Base.typeof(value_2) <: Core.Array || Base.typeof(value_2) <: Base.Vector) || Base.isa(value_2, Core.String))) && (Base.ismissing(value_2) || Base.isnothing(value_2))
                    stepping_data_2[key_1][key_2] = Base.string("null");
                elseif (!(Base.isa(value_2, Base.Dict) || (Base.typeof(value_2) <: Core.Array || Base.typeof(value_2) <: Base.Vector) || Base.isa(value_2, Core.String))) && ((Base.typeof(value_2) <: Core.Real) && Base.isinf(value_2) && (value_2 > 0.0))
                    stepping_data_2[key_1][key_2] = Base.string("+Infinity");
                elseif (!(Base.isa(value_2, Base.Dict) || (Base.typeof(value_2) <: Core.Array || Base.typeof(value_2) <: Base.Vector) || Base.isa(value_2, Core.String))) && ((Base.typeof(value_2) <: Core.Real) && Base.isinf(value_2) && (value_2 < 0.0))
                    stepping_data_2[key_1][key_2] = Base.string("-Infinity");
                elseif (!(Base.isa(value_2, Base.Dict) || (Base.typeof(value_2) <: Core.Array || Base.typeof(value_2) <: Base.Vector) || Base.isa(value_2, Core.String))) && ((Base.typeof(value_2) <: Dates.Date) || (Base.typeof(value_2) <: Dates.DateTime) || (Base.typeof(value_2) <: Dates.Time))
                    stepping_data_2[key_1][key_2] = Base.string(Dates.format(value_2, "yyyy-mm-dd"));
                else
...
# println(stepping_data_2);
Base.write(Base.stdout, Base.string(JSONstring(stepping_data_2)));  # 使用藉助第三方擴展包「JSON」裏的 JSON.json() 函數創建的自定義的 JSONstring() 函數將 Julia 字典（Dict）對象轉換爲 JSON 字符串;
