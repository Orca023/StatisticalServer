...
# using Revise;  # 導入第三方擴展包「Revise」，用於重載（熱加載）（Hot reload）模組（Module），需要在控制臺先安裝第三方擴展包「Revise」：julia> using Pkg; Pkg.add("Revise") 成功之後才能使用;
...
# 使用 Base.MainInclude.include() 函數可導入本地 Julia 脚本文檔到當前位置執行;
Base.MainInclude.include("./Quantitative_Indicators.jl");  # 加載自定義算法模組，導入本地自定義的日棒缐（K Line）趨勢指標計算模組;
Base.MainInclude.include("./Quantitative_MarketTiming.jl");  # 加載自定義算法模組，導入本地自定義的日棒缐（K Line）趨勢交易擇時計算模組;
Base.MainInclude.include("./Quantitative_PickStock.jl");  # 加載自定義算法模組，導入本地自定義的日棒缐（K Line）趨勢交易選股計算模組;
Base.MainInclude.include("./Quantitative_SizePosition.jl");  # 加載自定義算法模組，導入本地自定義的日棒缐（K Line）趨勢交易倉位計算模組;
Base.MainInclude.include("./Quantitative_BackTesting.jl");  # 加載自定義算法模組，導入本地自定義的日棒缐（K Line）趨勢交易步進（Stepper movement）回測計算模組;
...
function do_Request(request_Dict::Base.Dict{Core.String, Core.Any})::Core.String
    ...
    elseif request_Path === "/uploadFile"
        # 客戶端或瀏覽器請求 url = http://[::1]:10001/uploadFile?Key=username:password&algorithmUser=username&algorithmPass=password&fileName=JuliaServer.jl
        # 客戶端或瀏覽器請求 url = http://127.0.0.1:10001/uploadFile?Key=username:password&algorithmUser=username&algorithmPass=password&fileName=JuliaServer.jl
    ...
    elseif request_Path === "/deleteFile"
        # 客戶端或瀏覽器請求 url = http://[::1]:10001/deleteFile?Key=username:password&algorithmUser=username&algorithmPass=password&fileName=JuliaServer.jl
        # 客戶端或瀏覽器請求 url = http://127.0.0.1:10001/deleteFile?Key=username:password&algorithmUser=username&algorithmPass=password&fileName=JuliaServer.jl
    ...
    elseif request_Path === "/KLineCleaning"
        # 客戶端或瀏覽器請求 url = http://[::1]:10001/KLineCleaning?Key=username:password&algorithmUser=username&algorithmPass=password&algorithmName=KLineCleaning&configFile=C:/StatisticalServer/StatisticalServerJulia/config.txt&input_K_Line=C:/StatisticalServer/Data/K-Day-source/&is_save_JLD=true&output_jld_K_Line=C:/StatisticalServer/Data/steppingData.jld&is_save_csv=true&output_csv_K_Line=C:/StatisticalServer/Data/K-Day/&is_save_xlsx=false&output_xlsx_K_Line=C:/StatisticalServer/Data/K-Day/
        # 客戶端或瀏覽器請求 url = http://127.0.0.1:10001/KLineCleaning?Key=username:password&algorithmUser=username&algorithmPass=password&algorithmName=KLineCleaning&configFile=C:/StatisticalServer/StatisticalServerJulia/config.txt&input_K_Line=C:/StatisticalServer/Data/K-Day-source/&is_save_JLD=true&output_jld_K_Line=C:/StatisticalServer/Data/steppingData.jld&is_save_csv=true&output_csv_K_Line=C:/StatisticalServer/Data/K-Day/&is_save_xlsx=false&output_xlsx_K_Line=C:/StatisticalServer/Data/K-Day/
        ...
        # 預設參數初值;
        input_K_Line_Daily_file = Base.string(Base.replace(Base.string(Base.Filesystem.joinpath(Base.string(Base.dirname(Base.dirname(Base.dirname(Base.@__FILE__)))), "Data", "K-Day-source")), "\\" => "/"));  # "C:/StatisticalServer/Data/K-Day-source/";
        ...
        output_jld_K_Line_Daily_file = Base.string(Base.replace(Base.string(Base.Filesystem.joinpath(Base.string(Base.dirname(Base.dirname(Base.dirname(Base.@__FILE__)))), "Data", "steppingData.jld")), "\\" => "/"));  # "C:/StatisticalServer/Data/steppingData.jld";
        # print(output_jld_K_Line_Daily_file, "\n");
        is_save_csv = "false";  # true or false;
        output_csv_K_Line_Daily_file_dir = Base.string(Base.replace(Base.string(Base.Filesystem.joinpath(Base.string(Base.dirname(Base.dirname(Base.dirname(Base.@__FILE__)))), "Data", "K-Day")), "\\" => "/"));  # "C:/StatisticalServer/Data/K-Day/";
        # print(output_csv_K_Line_Daily_file_dir, "\n");
        is_save_xlsx = "false";  # true or false;
        output_xlsx_K_Line_Daily_file_dir = Base.string(Base.replace(Base.string(Base.Filesystem.joinpath(Base.string(Base.dirname(Base.dirname(Base.dirname(Base.@__FILE__)))), "Data", "K-Day")), "\\" => "/"));  # "C:/StatisticalServer/Data/K-Day/";
        # print(output_xlsx_K_Line_Daily_file_dir, "\n");
        Cleaned_K_Line_Daily_file_dir = Base.string(Base.replace(Base.string(Base.Filesystem.joinpath(Base.string(Base.dirname(Base.dirname(Base.dirname(Base.@__FILE__)))), "Data", "steppingData.jld")), "\\" => "/"));  # "C:/StatisticalServer/Data/steppingData.jld";
        # print(Cleaned_K_Line_Daily_file_dir, "\n");
        # 從配置文檔（./config.txt）讀取傳入參數值;
        Quantitative_Trading_configFile = Base.string(Base.replace(Base.string(Base.Filesystem.joinpath(Base.string(Base.dirname(Base.dirname(Base.@__FILE__))), "config.txt")), "\\" => "/"));  # "C:/StatisticalServer/StatisticalServerJulia/config.txt" # "/home/StatisticalServer/StatisticalServerJulia/config.txt";
        # Quantitative_Trading_configFile = Base.string(Base.replace(Base.string(Base.Filesystem.joinpath(Base.string(Base.Filesystem.abspath("..")), "config.txt")), "\\" => "/"));  # "C:/StatisticalServer/StatisticalServerJulia/config.txt" # "/home/StatisticalServer/StatisticalServerJulia/config.txt";
        # print(Quantitative_Trading_configFile, "\n");

        # argumentArray = Core.Array{Core.Union{Core.Bool, Core.Float64, Core.Int64, Core.String},1}();
        # if Base.haskey(request_data_Dict, "argumentKLineClear")
        #     if Base.length(request_data_Dict["argumentKLineClear"]) > 0
        #         for i = 1:Base.length(request_data_Dict["argumentKLineClear"])
        #             Base.push!(argumentArray, Base.string(request_data_Dict["argumentKLineClear"][i]));
        #         end
        #     end
        # end

        if Base.haskey(request_data_Dict, "configFile")
            if Base.length(request_data_Dict["configFile"]) > 0
                Quantitative_Trading_configFile = Base.string(request_data_Dict["configFile"][1]);
            end
        end
        # println("Post configFile = ", Quantitative_Trading_configFile);
        if Base.haskey(request_Url_Query_Dict, "configFile")
            Quantitative_Trading_configFile = Base.string(request_Url_Query_Dict["configFile"]);
        end
        # println("URL query configFile = ", Quantitative_Trading_configFile);

        if Base.haskey(request_data_Dict, "input_K_Line")
            if Base.length(request_data_Dict["input_K_Line"]) > 0
                input_K_Line_Daily_file = Base.string(request_data_Dict["input_K_Line"][1]);
            end
        end
        # println("Post input_K_Line = ", input_K_Line_Daily_file);
        if Base.haskey(request_Url_Query_Dict, "input_K_Line")
            input_K_Line_Daily_file = Base.string(request_Url_Query_Dict["input_K_Line"]);
        end
        # println("URL query input_K_Line = ", input_K_Line_Daily_file);

        if Base.haskey(request_data_Dict, "is_save_JLD")
            if Base.length(request_data_Dict["is_save_JLD"]) > 0
                # is_save_JLD = Base.string(request_data_Dict["is_save_JLD"][1]);
                # is_save_JLD = Base.parse(Core.Bool, Base.string(request_data_Dict["is_save_JLD"][1]));
                if request_data_Dict["is_save_JLD"][1] === "true" || request_data_Dict["is_save_JLD"][1] === "True" || request_data_Dict["is_save_JLD"][1] === "TRUE" || request_data_Dict["is_save_JLD"][1] === "1"
                    # is_save_JLD = true;
                    is_save_JLD = "true";
                end
                if request_data_Dict["is_save_JLD"][1] === "false" || request_data_Dict["is_save_JLD"][1] === "False" || request_data_Dict["is_save_JLD"][1] === "FALSE" || request_data_Dict["is_save_JLD"][1] === "0"
                    # is_save_JLD = false;
                    is_save_JLD = "false";
                end
            end
        end
        # println("Post is_save_JLD = ", is_save_JLD);
        if Base.haskey(request_Url_Query_Dict, "is_save_JLD")
            # is_save_JLD = Base.string(request_Url_Query_Dict["is_save_JLD"]);
            # is_save_JLD = Base.parse(Core.Bool, Base.string(request_Url_Query_Dict["is_save_JLD"]));
            if request_Url_Query_Dict["is_save_JLD"] === "true" || request_Url_Query_Dict["is_save_JLD"] === "True" || request_Url_Query_Dict["is_save_JLD"] === "TRUE" || request_Url_Query_Dict["is_save_JLD"] === "1"
                # is_save_JLD = true;
                is_save_JLD = "true";
            end
            if request_Url_Query_Dict["is_save_JLD"] === "false" || request_Url_Query_Dict["is_save_JLD"] === "False" || request_Url_Query_Dict["is_save_JLD"] === "FALSE" || request_Url_Query_Dict["is_save_JLD"] === "0"
                # is_save_JLD = false;
                is_save_JLD = "false";
            end
        end
        # println("URL query is_save_JLD = ", is_save_JLD);

        if Base.haskey(request_data_Dict, "output_jld_K_Line")
            if Base.length(request_data_Dict["output_jld_K_Line"]) > 0
                output_jld_K_Line_Daily_file = Base.string(request_data_Dict["output_jld_K_Line"][1]);
            end
        end
        # println("Post output_jld_K_Line = ", output_jld_K_Line_Daily_file);
        if Base.haskey(request_Url_Query_Dict, "output_jld_K_Line")
            output_jld_K_Line_Daily_file = Base.string(request_Url_Query_Dict["output_jld_K_Line"]);
        end
        # println("URL query output_jld_K_Line = ", output_jld_K_Line_Daily_file);

        if Base.haskey(request_data_Dict, "is_save_csv")
            if Base.length(request_data_Dict["is_save_csv"]) > 0
                # is_save_csv = Base.string(request_data_Dict["is_save_csv"][1]);
                # is_save_csv = Base.parse(Core.Bool, Base.string(request_data_Dict["is_save_csv"][1]));
                if request_data_Dict["is_save_csv"][1] === "true" || request_data_Dict["is_save_csv"][1] === "True" || request_data_Dict["is_save_csv"][1] === "TRUE" || request_data_Dict["is_save_csv"][1] === "1"
                    # is_save_csv = true;
                    is_save_csv = "true";
                end
                if request_data_Dict["is_save_csv"][1] === "false" || request_data_Dict["is_save_csv"][1] === "False" || request_data_Dict["is_save_csv"][1] === "FALSE" || request_data_Dict["is_save_csv"][1] === "0"
                    # is_save_csv = false;
                    is_save_csv = "false";
                end
            end
        end
        # println("Post is_save_csv = ", is_save_csv);
        if Base.haskey(request_Url_Query_Dict, "is_save_csv")
            # is_save_csv = Base.string(request_Url_Query_Dict["is_save_csv"]);
            # is_save_csv = Base.parse(Core.Bool, Base.string(request_Url_Query_Dict["is_save_csv"]));
            if request_Url_Query_Dict["is_save_csv"] === "true" || request_Url_Query_Dict["is_save_csv"] === "True" || request_Url_Query_Dict["is_save_csv"] === "TRUE" || request_Url_Query_Dict["is_save_csv"] === "1"
                # is_save_csv = true;
                is_save_csv = "true";
            end
            if request_Url_Query_Dict["is_save_csv"] === "false" || request_Url_Query_Dict["is_save_csv"] === "False" || request_Url_Query_Dict["is_save_csv"] === "FALSE" || request_Url_Query_Dict["is_save_csv"] === "0"
                # is_save_csv = false;
                is_save_csv = "false";
            end
        end
        # println("URL query is_save_csv = ", is_save_csv);

        if Base.haskey(request_data_Dict, "output_csv_K_Line")
            if Base.length(request_data_Dict["output_csv_K_Line"]) > 0
                output_csv_K_Line_Daily_file_dir = Base.string(request_data_Dict["output_csv_K_Line"][1]);
            end
        end
        # println("Post output_csv_K_Line = ", output_csv_K_Line_Daily_file_dir);
        if Base.haskey(request_Url_Query_Dict, "output_csv_K_Line")
            output_csv_K_Line_Daily_file_dir = Base.string(request_Url_Query_Dict["output_csv_K_Line"]);
        end
        # println("URL query output_csv_K_Line = ", output_csv_K_Line_Daily_file_dir);

        if Base.haskey(request_data_Dict, "is_save_xlsx")
            if Base.length(request_data_Dict["is_save_xlsx"]) > 0
                # is_save_xlsx = Base.string(request_data_Dict["is_save_xlsx"][1]);
                # is_save_xlsx = Base.parse(Core.Bool, Base.string(request_data_Dict["is_save_xlsx"][1]));
                if request_data_Dict["is_save_xlsx"][1] === "true" || request_data_Dict["is_save_xlsx"][1] === "True" || request_data_Dict["is_save_xlsx"][1] === "TRUE" || request_data_Dict["is_save_xlsx"][1] === "1"
                    # is_save_xlsx = true;
                    is_save_xlsx = "true";
                end
                if request_data_Dict["is_save_xlsx"][1] === "false" || request_data_Dict["is_save_xlsx"][1] === "False" || request_data_Dict["is_save_xlsx"][1] === "FALSE" || request_data_Dict["is_save_xlsx"][1] === "0"
                    # is_save_xlsx = false;
                    is_save_xlsx = "false";
                end
            end
        end
        # println("Post is_save_xlsx = ", is_save_xlsx);
        if Base.haskey(request_Url_Query_Dict, "is_save_xlsx")
            # is_save_xlsx = Base.string(request_Url_Query_Dict["is_save_xlsx"]);
            # is_save_xlsx = Base.parse(Core.Bool, Base.string(request_Url_Query_Dict["is_save_xlsx"]));
            if request_Url_Query_Dict["is_save_xlsx"] === "true" || request_Url_Query_Dict["is_save_xlsx"] === "True" || request_Url_Query_Dict["is_save_xlsx"] === "TRUE" || request_Url_Query_Dict["is_save_xlsx"] === "1"
                # is_save_xlsx = true;
                is_save_xlsx = "true";
            end
            if request_Url_Query_Dict["is_save_xlsx"] === "false" || request_Url_Query_Dict["is_save_xlsx"] === "False" || request_Url_Query_Dict["is_save_xlsx"] === "FALSE" || request_Url_Query_Dict["is_save_xlsx"] === "0"
                # is_save_xlsx = false;
                is_save_xlsx = "false";
            end
        end
        # println("URL query is_save_xlsx = ", is_save_xlsx);

        if Base.haskey(request_data_Dict, "output_xlsx_K_Line")
            if Base.length(request_data_Dict["output_xlsx_K_Line"]) > 0
                output_xlsx_K_Line_Daily_file_dir = Base.string(request_data_Dict["output_xlsx_K_Line"][1]);
            end
        end
        # println("Post output_xlsx_K_Line = ", output_xlsx_K_Line_Daily_file_dir);
        if Base.haskey(request_Url_Query_Dict, "output_xlsx_K_Line")
            output_xlsx_K_Line_Daily_file_dir = Base.string(request_Url_Query_Dict["output_xlsx_K_Line"]);
        end
        # println("URL query output_xlsx_K_Line = ", output_xlsx_K_Line_Daily_file_dir);

        if Base.haskey(request_data_Dict, "Cleaned_K_Line")
            if Base.length(request_data_Dict["Cleaned_K_Line"]) > 0
                Cleaned_K_Line_Daily_file_dir = Base.string(request_data_Dict["Cleaned_K_Line"][1]);
            end
        end
        # println("Post Cleaned_K_Line = ", Cleaned_K_Line_Daily_file_dir);
        if Base.haskey(request_Url_Query_Dict, "Cleaned_K_Line")
            Cleaned_K_Line_Daily_file_dir = Base.string(request_Url_Query_Dict["Cleaned_K_Line"]);
        end
        # println("URL query Cleaned_K_Line = ", Cleaned_K_Line_Daily_file_dir);


        # # 向外部 .jl 脚本文檔裏的變量賦新值;
        # # Base.MainInclude.include("./Quantitative_Data_Cleaning.jl");  # 以脚本形式加載自定義算法模組，導入本地自定義的日棒缐（K Line）數據清洗功能模組;
        # import .Quantitative_Data_Cleaning  # 以模組形式導入本地自定義的日棒缐（K Line）數據清洗功能模組;
        # Quantitative_Data_Cleaning.input_K_Line_Daily_file = Base.string(Base.Filesystem.joinpath(Base.string(Base.dirname(Base.dirname(Base.dirname(Base.@__FILE__)))), "Data", "K-Day-source"));
        ...
        # # print(Quantitative_Data_Cleaning.input_K_Line_Daily_file, "\n");
        # Quantitative_Data_Cleaning.is_save_JLD = false;  # true or false;
        # Quantitative_Data_Cleaning.is_save_JLD = Base.parse(Core.Bool, is_save_JLD);
        # Quantitative_Data_Cleaning.output_jld_K_Line_Daily_file = Base.string(Base.Filesystem.joinpath(Base.string(Base.dirname(Base.dirname(Base.dirname(Base.@__FILE__)))), "Data", "steppingData.jld"));  # "C:/StatisticalServer/Data/steppingData.jld";
        # Quantitative_Data_Cleaning.output_jld_K_Line_Daily_file = output_jld_K_Line_Daily_file;
        # # print(Quantitative_Data_Cleaning.output_jld_K_Line_Daily_file, "\n");
        # Quantitative_Data_Cleaning.is_save_csv = false;  # true or false;
        # Quantitative_Data_Cleaning.is_save_csv = Base.parse(Core.Bool, is_save_csv);
        # Quantitative_Data_Cleaning.output_csv_K_Line_Daily_file_dir = Base.string(Base.Filesystem.joinpath(Base.string(Base.dirname(Base.dirname(Base.dirname(Base.@__FILE__)))), "Data", "K-Day"));  # "C:/StatisticalServer/Data/K-Day/";
        # Quantitative_Data_Cleaning.output_csv_K_Line_Daily_file_dir = output_csv_K_Line_Daily_file_dir;
        # # print(Quantitative_Data_Cleaning.output_csv_K_Line_Daily_file_dir, "\n");
        # Quantitative_Data_Cleaning.is_save_xlsx = false;  # true or false;
        # Quantitative_Data_Cleaning.is_save_xlsx = Base.parse(Core.Bool, is_save_xlsx);
        # Quantitative_Data_Cleaning.output_xlsx_K_Line_Daily_file_dir = Base.string(Base.Filesystem.joinpath(Base.string(Base.dirname(Base.dirname(Base.dirname(Base.@__FILE__)))), "Data", "K-Day"));  # "C:/StatisticalServer/Data/K-Day/";
        # Quantitative_Data_Cleaning.output_xlsx_K_Line_Daily_file_dir = output_xlsx_K_Line_Daily_file_dir
        # # print(Quantitative_Data_Cleaning.output_xlsx_K_Line_Daily_file_dir, "\n");
        # # 從配置文檔（./config.txt）讀取傳入參數值;
        # Quantitative_Data_Cleaning.configFile = Base.string(Base.Filesystem.joinpath(Base.string(Base.dirname(Base.dirname(Base.@__FILE__))), "config.txt"));  # "C:/StatisticalServer/StatisticalServerJulia/config.txt" # "/home/StatisticalServer/StatisticalServerJulia/config.txt";
        # Quantitative_Data_Cleaning.configFile = Quantitative_Trading_configFile;
        # # Quantitative_Data_Cleaning.configFile = Base.string(Base.replace(Base.string(Base.Filesystem.joinpath(Base.string(Base.dirname(Base.dirname(Base.@__FILE__))), "config.txt")), "\\" => "/"));  # "C:/StatisticalServer/StatisticalServerJulia/config.txt" # "/home/StatisticalServer/StatisticalServerJulia/config.txt";
        # # Quantitative_Data_Cleaning.configFile = Base.string(Base.Filesystem.joinpath(Base.string(Base.Filesystem.abspath("..")), "config.txt"));  # "C:/StatisticalServer/StatisticalServerJulia/config.txt" # "/home/StatisticalServer/StatisticalServerJulia/config.txt";
        # # Quantitative_Data_Cleaning.configFile = Base.string(Base.replace(Base.string(Base.Filesystem.joinpath(Base.string(Base.Filesystem.abspath("..")), "config.txt")), "\\" => "/"));  # "C:/StatisticalServer/StatisticalServerJulia/config.txt" # "/home/StatisticalServer/StatisticalServerJulia/config.txt";
        # # print(Quantitative_Data_Cleaning.configFile, "\n");
        # reload("Quantitative_Data_Cleaning")  # 熱重載熱更新，重新以模組形式導入本地自定義的日棒缐（K Line）數據清洗功能模組;
        # # Base.MainInclude.include("./Quantitative_Data_Cleaning.jl");  # 重新更新以脚本形式加載自定義算法模組，導入本地自定義的日棒缐（K Line）數據清洗功能模組;
        ...
        # 控制臺命令列運行指令：
        # C:\> C:/StatisticalServer/Julia/Julia-1.9.3/bin/julia.exe -p 4 --project=C:/StatisticalServer/StatisticalServerJulia/ C:/StatisticalServer/StatisticalServerJulia/src/Quantitative_Data_Cleaning.jl configFile=C:/StatisticalServer/StatisticalServerJulia/config.txt input_K_Line=C:/StatisticalServer/Data/K-Day-source/ is_save_JLD=true output_jld_K_Line=C:/StatisticalServer/Data/steppingData.jld is_save_csv=false output_csv_K_Line=C:/StatisticalServer/Data/K-Day/ is_save_xlsx=false output_xlsx_K_Line=C:/StatisticalServer/Data/K-Day/
        # root@localhost:~# /usr/julia/julia-1.10.3/bin/julia -p 4 --project=/home/StatisticalServer/StatisticalServerJulia/ /home/StatisticalServer/StatisticalServerJulia/src/Quantitative_Data_Cleaning.jl configFile=/home/StatisticalServer/StatisticalServerJulia/config.txt input_K_Line=/home/StatisticalServer/Data/K-Day-source/ is_save_JLD=false output_jld_K_Line=/home/StatisticalServer/Data/steppingData.jld is_save_csv=false output_csv_K_Line=/home/StatisticalServer/Data/K-Day/ is_save_xlsx=false output_xlsx_K_Line=/home/StatisticalServer/Data/K-Day/
        # 啓動運行;
        # 參數 -p 4 表示設定允許 4 個進程并發;
        # 參數 --project=C:/StatisticalServer/StatisticalServerJulia/ 表示隔離全局環境，只是用項目私人環境 C:/StatisticalServer/StatisticalServerJulia/ 啓動 Julia 解釋器;

        Julia_exe_path = Base.string(Base.replace(Base.string(Base.Filesystem.joinpath(Base.string(Base.Filesystem.abspath(Base.Sys.BINDIR)), "julia.exe")), "\\" => "/"));  # C:/StatisticalServer/Julia/Julia-1.9.3/bin/julia.exe
        # print(Julia_exe_path, "\n");  # C:/StatisticalServer/Julia/Julia-1.9.3/bin/julia.exe
        Julia_program_path = Base.string(Base.replace(Base.string(Base.dirname(Base.Filesystem.abspath(Base.active_project()))), "\\" => "/"));  # C:/StatisticalServer/StatisticalServerJulia/
        # print(Julia_program_path, "\n");  # C:/StatisticalServer/StatisticalServerJulia/
        Julia_script_path = Base.string(Base.replace(Base.string(Base.Filesystem.joinpath(Base.string(Base.dirname(Base.Filesystem.abspath(Base.@__FILE__))), "Quantitative_Data_Cleaning.jl")), "\\" => "/"));  # C:/StatisticalServer/StatisticalServerJulia/src/Quantitative_Data_Cleaning.jl
        # print(Julia_script_path, "\n");  # C:/StatisticalServer/StatisticalServerJulia/src/Quantitative_Data_Cleaning.jl

        # 使用 Base.run() 函數從控制臺命令行運行外部 Linux Ubuntu shell 或 Windows cmd 指令程式;
        # Command = `$Julia_exe_path --project=$Julia_program_path $Julia_script_path configFile=$Quantitative_Trading_configFile input_K_Line=$input_K_Line_Daily_file is_save_JLD=$is_save_JLD output_jld_K_Line=$output_jld_K_Line_Daily_file is_save_csv=$is_save_csv output_csv_K_Line=$output_csv_K_Line_Daily_file_dir is_save_xlsx=$is_save_xlsx output_xlsx_K_Line=$output_xlsx_K_Line_Daily_file_dir`;
        # Proc = Base.open(Command, "w", Base.stdout);
        # stdout_Command = Base.read(Proc, Core.String);
        # println(Base.typeof(stdout_Command))
        # println(Base.chomp(stdout_Command));  # Base.chomp() 刪除字符串的單個尾隨換行符"\n"；
        # println(stdout_Command)
        # shell_script = "$Julia_exe_path --project=$Julia_program_path $Julia_script_path configFile=$Quantitative_Trading_configFile input_K_Line=$input_K_Line_Daily_file is_save_JLD=$is_save_JLD output_jld_K_Line=$output_jld_K_Line_Daily_file is_save_csv=$is_save_csv output_csv_K_Line=$output_csv_K_Line_Daily_file_dir is_save_xlsx=$is_save_xlsx output_xlsx_K_Line=$output_xlsx_K_Line_Daily_file_dir";
        shell_script = "$Julia_exe_path --project=$Julia_program_path $Julia_script_path configFile=$Quantitative_Trading_configFile input_K_Line=$input_K_Line_Daily_file is_save_JLD=$is_save_JLD output_jld_K_Line=$output_jld_K_Line_Daily_file is_save_csv=$is_save_csv output_csv_K_Line=$output_csv_K_Line_Daily_file_dir is_save_xlsx=$is_save_xlsx output_xlsx_K_Line=$output_xlsx_K_Line_Daily_file_dir";
        # shell_script = "C:/StatisticalServer/Julia/Julia-1.9.3/bin/julia.exe --project=C:/StatisticalServer/StatisticalServerJulia/ C:/StatisticalServer/StatisticalServerJulia/src/Quantitative_Data_Cleaning.jl configFile=C:/StatisticalServer/StatisticalServerJulia/config.txt input_K_Line=C:/StatisticalServer/Data/K-Day-source/ is_save_JLD=false output_jld_K_Line=C:/StatisticalServer/Data/steppingData.jld is_save_csv=false output_csv_K_Line=C:/StatisticalServer/Data/K-Day/ is_save_xlsx=false output_xlsx_K_Line=C:/StatisticalServer/Data/K-Day/";
        # shell_script = "/usr/julia/julia-1.6.7/bin/julia --project=/home/StatisticalServer/StatisticalServerJulia/ /home/StatisticalServer/StatisticalServerJulia/src/Quantitative_Data_Cleaning.jl configFile=/home/StatisticalServer/StatisticalServerJulia/config.txt input_K_Line=/home/StatisticalServer/Data/K-Day-source/ is_save_JLD=false output_jld_K_Line=/home/StatisticalServer/Data/steppingData.jld is_save_csv=false output_csv_K_Line=/home/StatisticalServer/Data/K-Day/ is_save_xlsx=false output_xlsx_K_Line=/home/StatisticalServer/Data/K-Day/";
        # println(shell_script);
        if Base.Sys.iswindows()
            # stdout_Command = Base.run(`cmd /c dir`);
            Command = `cmd /c $shell_script`;
            # Command = "cmd /c C:/StatisticalServer/Julia/Julia-1.9.3/bin/julia.exe --project=C:/StatisticalServer/StatisticalServerJulia/ C:/StatisticalServer/StatisticalServerJulia/src/Quantitative_Data_Cleaning.jl configFile=C:/StatisticalServer/StatisticalServerJulia/config.txt input_K_Line=C:/StatisticalServer/Data/K-Day-source/ is_save_JLD=false output_jld_K_Line=C:/StatisticalServer/Data/steppingData.jld is_save_csv=false output_csv_K_Line=C:/StatisticalServer/Data/K-Day/ is_save_xlsx=false output_xlsx_K_Line=C:/StatisticalServer/Data/K-Day/";
            stdout_Command = Base.read(Command, Core.String);
        elseif Base.Sys.islinux()
            Command = `bash -c $shell_script`;
            # Command = "bash -c /usr/julia/julia-1.6.7/bin/julia --project=/home/StatisticalServer/StatisticalServerJulia/ /home/StatisticalServer/StatisticalServerJulia/src/Quantitative_Data_Cleaning.jl configFile=/home/StatisticalServer/StatisticalServerJulia/config.txt input_K_Line=/home/StatisticalServer/Data/K-Day-source/ is_save_JLD=false output_jld_K_Line=/home/StatisticalServer/Data/steppingData.jld is_save_csv=false output_csv_K_Line=/home/StatisticalServer/Data/K-Day/ is_save_xlsx=false output_xlsx_K_Line=/home/StatisticalServer/Data/K-Day/";
        end
        # Proc = Base.open(Command, "w", Base.stdout);
        # Base.run(command, args...; wait::Bool = true)
        # stdout_Command = Base.run(Command);
        # stdout_Command = Base.run(`cmd /c dir`);
        # stdout_Command = Base.read(Proc, Core.String);
        stdout_Command = Base.read(Command, Core.String);
        # stdout_Command = Base.read("cmd /c $Julia_exe_path --project=$Julia_program_path $Julia_script_path configFile=$Quantitative_Trading_configFile input_K_Line=$input_K_Line_Daily_file is_save_JLD=$is_save_JLD output_jld_K_Line=$output_jld_K_Line_Daily_file is_save_csv=$is_save_csv output_csv_K_Line=$output_csv_K_Line_Daily_file_dir is_save_xlsx=$is_save_xlsx output_xlsx_K_Line=$output_xlsx_K_Line_Daily_file_dir", Core.String);
        # stdout_Command = Base.read("cmd /c C:/StatisticalServer/Julia/Julia-1.9.3/bin/julia.exe --project=C:/StatisticalServer/StatisticalServerJulia/ C:/StatisticalServer/StatisticalServerJulia/src/Quantitative_Data_Cleaning.jl configFile=C:/StatisticalServer/StatisticalServerJulia/config.txt input_K_Line=C:/StatisticalServer/Data/K-Day-source/ is_save_JLD=false output_jld_K_Line=C:/StatisticalServer/Data/steppingData.jld is_save_csv=false output_csv_K_Line=C:/StatisticalServer/Data/K-Day/ is_save_xlsx=false output_xlsx_K_Line=C:/StatisticalServer/Data/K-Day/", Core.String);
        # println(Base.typeof(stdout_Command));
        # println(Base.chomp(stdout_Command));  # Base.chomp() 刪除字符串的單個尾隨換行符"\n"；
        # Base.write(Base.stdout, stdout_Command);
        # Base.write(Base.stdout, Base.split(stdout_Command, "\n"));

        stdout_Command_head = "";
        stdout_Command_body = "";
        if Base.length(stdout_Command) > 0
            if Base.occursin("\r\n\r\n", stdout_Command)
                stdout_Command_head = Base.convert(Core.String, Base.split(stdout_Command, "\r\n\r\n")[1]);  # 使用 convert() 函數將子字符串(SubString)型轉換為字符串(String)型變量;
                stdout_Command_body = Base.convert(Core.String, Base.split(stdout_Command, "\r\n\r\n")[2]);
            else
                stdout_Command_head = stdout_Command;
                stdout_Command_body = stdout_Command;
            end
        end
        # print(stdout_Command_head * "\n");
        # print(stdout_Command_body * "\n");

        backArray = Core.Array{Core.Union{Core.Bool, Core.Float64, Core.Int64, Core.String},1}();  # 聲明一個聯合類型的空1維數組;
        if Base.length(stdout_Command_head) > 0
            if Base.occursin("\n", stdout_Command_head)
                for x in Base.split(stdout_Command_head, "\n")
                    # print(x * "\n");
                    x = Base.convert(Core.String, x);  # 使用 convert() 函數將子字符串(SubString)型轉換為字符串(String)型變量;
                    Base.push!(backArray, x);  # 使用 push! 函數在數組末尾追加推入新元素;
                    # Base.write(Base.stdout, x * "\n");
                end
            else
                Base.push!(backArray, stdout_Command_head);
            end
        end
        # println(backArray);

        response_body_Dict = Base.Dict{Core.String, Core.Any}();
        response_body_Dict["configFile"] = Base.string(Quantitative_Trading_configFile);
        response_body_Dict["input_K_Line"] = Base.string(input_K_Line_Daily_file);
        response_body_Dict["is_save_JLD"] = Base.string(is_save_JLD);
        response_body_Dict["output_jld_K_Line"] = Base.string(output_jld_K_Line_Daily_file);
        response_body_Dict["is_save_csv"] = Base.string(is_save_csv);
        response_body_Dict["output_csv_K_Line"] = Base.string(output_csv_K_Line_Daily_file_dir);
        response_body_Dict["is_save_xlsx"] = Base.string(is_save_xlsx);
        response_body_Dict["output_xlsx_K_Line"] = Base.string(output_xlsx_K_Line_Daily_file_dir);

        response_body_Dict["return_KLineCleaning"] = Base.Dict{Core.String, Core.Any}("KLineCleaned" => Base.Dict{Core.String, Core.Any}());
        stepping_data = Base.Dict{Core.String, Core.Any}();
        # stepping_data = Quantitative_Data_Cleaning.stepping_data;
        if Base.length(stdout_Command_body) > 0
            stepping_data = JSONparse(stdout_Command_body);  # 使用自定義函數 JSONparse() 將函數返回值的 JSON 字符串類型轉換為 Julia 字典（Dict）對象;
        end
        if Base.isa(stepping_data, Base.Dict) && Base.length(stepping_data) > 0
            for (key, value) in stepping_data
                # println("Key: $key, Value:\n$value");
                response_body_Dict["return_KLineCleaning"]["KLineCleaned"][key] = value;
            end
        end
        ...
        response_body_Dict["Server_say"] = Base.string(stdout_Command);  # Base.Dict("Server_say" => Base.string(request_POST_String));
        response_body_Dict["error"] = Base.string("");  # Base.Dict("Server_say" => Base.string(request_POST_String));
        # println(response_body_Dict);
        ...
        # response_body_Dict = Base.Dict{Core.String, Core.Any}(
        #     "configFile" => "C:/StatisticalServer/StatisticalServerJulia/config.txt",
        ...
        #     "stepping_data" => Base.Dict{Core.String, Core.Any}(
        #         "002611" => Base.Dict{Core.String, Core.Any}(
        #             "capitalization" => Core.Int64(12170000000),
        #             "date_transaction" => [Dates.Date("2019-1-2"), Dates.Date("2019-1-3"), Dates.Date("2019-1-4"), Dates.Date("2019-1-7"), Dates.Date("2019-1-8"), Dates.Date("2019-1-9"), Dates.Date("2019-1-10"), Dates.Date("2019-1-11"), Dates.Date("2019-1-14"), Dates.Date("2019-1-15"), Dates.Date("2019-1-16"), ...],
        #             "turnover_volume" => [Core.Int64(7385675), Core.Int64(33846475), Core.Int64(27957054), Core.Int64(25700917), Core.Int64(56678844), Core.Int64(39931296), Core.Int64(21818792), Core.Int64(15243953), Core.Int64(46110961), Core.Int64(29099424), Core.Int64(58411670), ...],
        #             "turnover_rate" => [Core.Float64(), Core.Float64(), Core.Float64(), ...],
        #             "turnover_amount" => [Core.Float64(27770014), Core.Float64(135627968), Core.Float64(109496376), Core.Float64(103257416), Core.Float64(242614176), Core.Float64(170208784), Core.Float64(90564944), Core.Float64(62862520), Core.Float64(197721472), Core.Float64(125331136), Core.Float64(260167456), ...],
        #             "opening_price" => [Core.Float64(3.75), Core.Float64(3.76), Core.Float64(3.84), Core.Float64(3.99), Core.Float64(4.04), Core.Float64(4.24), Core.Float64(4.12), Core.Float64(4.13), Core.Float64(4.14), Core.Float64(4.27), Core.Float64(4.32), ...],
        #             "close_price" => [Core.Float64(3.76), Core.Float64(3.94), Core.Float64(3.98), Core.Float64(4.05), Core.Float64(4.27), Core.Float64(4.18), Core.Float64(4.14), Core.Float64(4.13), Core.Float64(4.31), Core.Float64(4.34), Core.Float64(4.48), ...],
        #             "low_price" => [Core.Float64(3.73), Core.Float64(3.72), Core.Float64(3.8), Core.Float64(3.92), Core.Float64(3.98), Core.Float64(4.16), Core.Float64(4.11), Core.Float64(4.09), Core.Float64(4.12), Core.Float64(4.24), Core.Float64(4.29), ...],
        #             "high_price" => [Core.Float64(3.8), Core.Float64(4.14), Core.Float64(4.0), Core.Float64(4.07), Core.Float64(4.46), Core.Float64(4.36), Core.Float64(4.21), Core.Float64(4.17), Core.Float64(4.45), Core.Float64(4.36), Core.Float64(4.67), ...],
        #             "focus" => [Core.Float64(3.76), Core.Float64(3.89), Core.Float64(3.905), Core.Float64(4.0075), Core.Float64(4.1875), Core.Float64(4.235), Core.Float64(4.145), Core.Float64(4.13), Core.Float64(4.255), Core.Float64(4.3025), Core.Float64(4.44), ...],
        #             "amplitude" => [Core.Float64(0.029439203), Core.Float64(0.192180471), Core.Float64(0.099833194), Core.Float64(0.06751543), Core.Float64(0.220510771), Core.Float64(0.09), Core.Float64(0.045092498), Core.Float64(0.032659863), Core.Float64(0.155456318), Core.Float64(0.056789083), Core.Float64(0.174547033), ...],
        #             "amplitude_rate" => [Core.Float64(0.007829575), Core.Float64(0.04940372), Core.Float64(0.025565479), Core.Float64(0.016847269), Core.Float64(0.052659289), Core.Float64(0.021251476), Core.Float64(0.010878769), Core.Float64(0.007907957), Core.Float64(0.036534975), Core.Float64(0.01319909), Core.Float64(0.039312395), ...],
        #             "opening_price_Standardization" => [Core.Float64(-0.33968311), Core.Float64(-0.676447505), Core.Float64(-0.651086049), Core.Float64(-0.259200007), Core.Float64(-0.668901567), Core.Float64(0.055555556), Core.Float64(-0.554415953), Core.Float64(0.0), Core.Float64(-0.739757649), Core.Float64(-0.572293089), Core.Float64(-0.687493784), ...],
        #             "closing_price_Standardization" => [Core.Float64(0.0), Core.Float64(0.260172117), Core.Float64(0.751253134), Core.Float64(0.629485731), Core.Float64(0.374131385), Core.Float64(-0.611111111), Core.Float64(-0.110883191), Core.Float64(0.0), Core.Float64(0.353797136), Core.Float64(0.66033818), Core.Float64(0.229164595), ...],
        #             "low_price_Standardization" => [Core.Float64(-1.019049331), Core.Float64(-0.884585199), Core.Float64(-1.051754387), Core.Float64(-1.296000034), Core.Float64(-0.940997119), Core.Float64(-0.833333333), Core.Float64(-0.776182335), Core.Float64(-1.224744871), Core.Float64(-0.868411153), Core.Float64(-1.100563633), Core.Float64(-0.859367229), ...],
        #             "high_price_Standardization" => [Core.Float64(1.358732441), Core.Float64(1.300860587), Core.Float64(0.951587303), Core.Float64(0.92571431), Core.Float64(1.235767301), Core.Float64(1.388888889), Core.Float64(1.441481478), Core.Float64(1.224744871), Core.Float64(1.254371666), Core.Float64(1.012518542), Core.Float64(1.317696418), ...],
        #             "turnover_volume_growth_rate" => [Core.Float64(0.0), Core.Float64(3.58271925), Core.Float64(-0.174003969), Core.Float64(-0.080700098), Core.Float64(1.205323802), Core.Float64(-0.295481467), Core.Float64(-0.453591689), Core.Float64(-0.30133836), Core.Float64(2.024869009), Core.Float64(-0.3689261), Core.Float64(1.007313616), ...],
        #             "opening_price_growth_rate" => [Core.Float64(0.0), Core.Float64(0.002666667), Core.Float64(0.021276596), Core.Float64(0.0390625), Core.Float64(0.012531328), Core.Float64(0.04950495), Core.Float64(-0.028301887), Core.Float64(0.002427184), Core.Float64(0.002421308), Core.Float64(0.031400966), Core.Float64(0.011709602), ...],
        #             "closing_price_growth_rate" => [Core.Float64(0.0), Core.Float64(0.04787234), Core.Float64(0.010152284), Core.Float64(0.01758794), Core.Float64(0.054320988), Core.Float64(-0.021077283), Core.Float64(-0.009569378), Core.Float64(-0.002415459), Core.Float64(0.043583535), Core.Float64(0.006960557), Core.Float64(0.032258065), ...],
        #             "high_price_proportion" => [Core.Float64(0.989473684), Core.Float64(0.951690821), Core.Float64(0.995), Core.Float64(0.995085995), Core.Float64(0.957399103), Core.Float64(0.972477064), Core.Float64(0.983372922), Core.Float64(0.990407674), Core.Float64(0.968539326), Core.Float64(0.995412844), Core.Float64(0.959314775), ...],
        #             "low_price_proportion" => [Core.Float64(0.994666667), Core.Float64(0.989361702), Core.Float64(0.989583333), Core.Float64(0.98245614), Core.Float64(0.985148515), Core.Float64(0.995215311), Core.Float64(0.997572816), Core.Float64(0.99031477), Core.Float64(0.995169082), Core.Float64(0.992974239), Core.Float64(0.993055556), ...],
        #             "closing_minus_opening_price_growth_rate" => [Core.Float64(0.002666667), Core.Float64(0.04787234), Core.Float64(0.036458333), Core.Float64(0.015037594), Core.Float64(0.056930693), Core.Float64(-0.014150943), Core.Float64(0.004854369), Core.Float64(0.0), Core.Float64(0.041062802), Core.Float64(0.016393443), Core.Float64(0.037037037), ...],
        #             "book_value_per_share" => [Core.Float64(), Core.Float64(), Core.Float64(), ...],
        #             "price_earnings" => [Core.Float64(), Core.Float64(), Core.Float64(), ...],
        #             "sum_2_turnover_volume_growth_rate" => [Base.missing, Core.Float64(1.791359625), Core.Float64(0.808677828), Core.Float64(-0.167702083), Core.Float64(0.582486877), Core.Float64(0.153590217), Core.Float64(-0.601332423), Core.Float64(-0.528134205), Core.Float64(0.937099914), Core.Float64(0.321754202), Core.Float64(0.411425283), ...],
        #             "sum_2_opening_price_growth_rate" => [Base.missing, Core.Float64(0.001333333), Core.Float64(0.022609929), Core.Float64(0.049700798), Core.Float64(0.032062578), Core.Float64(0.055770615), Core.Float64(-0.001774706), Core.Float64(-0.005861879), Core.Float64(0.0036349), Core.Float64(0.03261162), Core.Float64(0.027410085), ...],
        #             "sum_2_closing_price_growth_rate" => [Base.missing, Core.Float64(0.02393617), Core.Float64(0.034088454), Core.Float64(0.022664082), Core.Float64(0.063114958), Core.Float64(0.003041605), Core.Float64(-0.02010802), Core.Float64(-0.007200148), Core.Float64(0.021187903), Core.Float64(0.028752324), Core.Float64(0.035738343), ...],
        #             "sum_2_closing_minus_opening_price_growth_rate" => [Base.missing, Core.Float64(0.049205674), Core.Float64(0.060394504), Core.Float64(0.033266761), Core.Float64(0.06444949), Core.Float64(0.007157202), Core.Float64(-0.001110551), Core.Float64(0.001213592), Core.Float64(0.020531401), Core.Float64(0.036924844), Core.Float64(0.045233758), ...],
        #             "sum_2_high_price_proportion" => [Base.missing, Core.Float64(1.446427663), Core.Float64(1.470845411), Core.Float64(1.492585995), Core.Float64(1.454942101), Core.Float64(1.451176616), Core.Float64(1.469611454), Core.Float64(1.482094135), Core.Float64(1.463743163), Core.Float64(1.479682507), Core.Float64(1.457021197), ...],
        #             "sum_2_low_price_proportion" => [Base.missing, Core.Float64(1.486695035), Core.Float64(1.484264184), Core.Float64(1.477247807), Core.Float64(1.476376585), Core.Float64(1.487789568), Core.Float64(1.495180471), Core.Float64(1.489101178), Core.Float64(1.490326467), Core.Float64(1.49055878), Core.Float64(1.489542675), ...],
        #             "sum_2_KLine_Intuitive_Momentum" => [Base.missing, Core.Float64(1.794033435), Core.Float64(0.816153433), Core.Float64(-0.162394081), Core.Float64(0.595663278), Core.Float64(0.154499026), Core.Float64(-0.601279023), Core.Float64(-0.52816919), Core.Float64(0.938211686), Core.Float64(0.326751665), Core.Float64(0.417624603), ...],
        #             "sum_3_turnover_volume_growth_rate" => [Base.missing, Base.missing, Core.Float64(0.73815851), Core.Float64(0.266944754), Core.Float64(0.327240342), Core.Float64(0.052928734), Core.Float64(-0.299794689), Core.Float64(-0.702226642), Core.Float64(0.440230023), Core.Float64(0.137056078), Core.Float64(1.039529723), ...],
        #             "sum_3_opening_price_growth_rate" => [Base.missing, Base.missing, Core.Float64(0.015369582), Core.Float64(0.054135786), Core.Float64(0.045665194), Core.Float64(0.070880003), Core.Float64(0.015352978), Core.Float64(0.006329915), Core.Float64(-0.0004517), Core.Float64(0.033824233), Core.Float64(0.033450682), ...],
        #             "sum_3_closing_price_growth_rate" => [Base.missing, Base.missing, Core.Float64(0.028044785), Core.Float64(0.040313576), Core.Float64(0.069430376), Core.Float64(0.021025331), Core.Float64(-0.009711602), Core.Float64(-0.015820805), Core.Float64(0.011327779), Core.Float64(0.023742447), Core.Float64(0.051426281), ...],
        #             "sum_3_closing_minus_opening_price_growth_rate" => [Base.missing, Base.missing, Core.Float64(0.069262116), Core.Float64(0.055300596), Core.Float64(0.079108534), Core.Float64(0.023927237), Core.Float64(0.012742857), Core.Float64(-0.000493578), Core.Float64(0.02845395), Core.Float64(0.029179096), Core.Float64(0.061653599), ...],
        #             "sum_3_high_price_proportion" => [Base.missing, Base.missing, Core.Float64(1.959285109), Core.Float64(1.975649602), Core.Float64(1.952456433), Core.Float64(1.942438465), Core.Float64(1.950823999), Core.Float64(1.970148643), Core.Float64(1.956602082), Core.Float64(1.971241619), Core.Float64(1.94576978), ...],
        #             "sum_3_low_price_proportion" => [Base.missing, Base.missing, Core.Float64(1.980713357), Core.Float64(1.971965597), Core.Float64(1.969980386), Core.Float64(1.979466368), Core.Float64(1.989432528), Core.Float64(1.987101751), Core.Float64(1.987903201), Core.Float64(1.986525217), Core.Float64(1.986761409), ...],
        #             "sum_3_KLine_Intuitive_Momentum" => [Base.missing, Base.missing, Core.Float64(0.74982791), Core.Float64(0.287293521), Core.Float64(0.362261072), Core.Float64(0.061384027), Core.Float64(-0.299515692), Core.Float64(-0.702208303), Core.Float64(0.441433708), Core.Float64(0.143633821), Core.Float64(1.059759237), ...],
        #             "sum_5_turnover_volume_growth_rate" => [Base.missing, Base.missing, Base.missing, Base.missing, Core.Float64(0.987779617), Core.Float64(0.424219288), Core.Float64(-0.461007301), Core.Float64(-0.589686585), Core.Float64(0.527522535), Core.Float64(-0.308230627), Core.Float64(0.585057271), ...],
        #             "sum_5_opening_price_growth_rate" => [Base.missing, Base.missing, Base.missing, Base.missing, Core.Float64(0.046091162), Core.Float64(0.092011485), Core.Float64(0.047942084), Core.Float64(0.031435847), Core.Float64(0.017940814), Core.Float64(0.033492099), Core.Float64(0.030271151), ...],
        #             "sum_5_closing_price_growth_rate" => [Base.missing, Base.missing, Base.missing, Base.missing, Core.Float64(0.074905317), Core.Float64(0.049900492), Core.Float64(0.014422453), Core.Float64(-0.003532006), Core.Float64(0.012116149), Core.Float64(0.011035464), Core.Float64(0.037233955), ...],
        #             "sum_5_closing_minus_opening_price_growth_rate" => [Base.missing, Base.missing, Base.missing, Base.missing, Core.Float64(0.110518038), Core.Float64(0.060149741), Core.Float64(0.03959144), Core.Float64(0.016099861), Core.Float64(0.032084862), Core.Float64(0.030145221), Core.Float64(0.060608277), ...],
        #             "sum_5_high_price_proportion" => [Base.missing, Base.missing, Base.missing, Base.missing, Core.Float64(2.929038965), Core.Float64(2.923786108), Core.Float64(2.932828433), Core.Float64(2.94256909), Core.Float64(2.931359864), Core.Float64(2.952333491), Core.Float64(2.9296063), ...],
        #             "sum_5_low_price_proportion" => [Base.missing, Base.missing, Base.missing, Base.missing, Core.Float64(2.959541441), Core.Float64(2.966513481), Core.Float64(2.975733296), Core.Float64(2.976052843), Core.Float64(2.981080415), Core.Float64(2.981370555), Core.Float64(2.980176867), ...],
        #             "sum_5_KLine_Intuitive_Momentum" => [Base.missing, Base.missing, Base.missing, Base.missing, Core.Float64(1.103698843), Core.Float64(0.498255593), Core.Float64(-0.439458671), Core.Float64(-0.585752412), Core.Float64(0.535949828), Core.Float64(-0.296415761), Core.Float64(0.620777963), ...],
        #             "sum_7_turnover_volume_growth_rate" => [Core.Float64(), Core.Float64(), Core.Float64(), ...],
        #             "sum_7_opening_price_growth_rate" => [Core.Float64(), Core.Float64(), Core.Float64(), ...],
        #             "sum_7_closing_price_growth_rate" => [Core.Float64(), Core.Float64(), Core.Float64(), ...],
        #             "sum_7_high_price_proportion" => [Core.Float64(), Core.Float64(), Core.Float64(), ...],
        #             "sum_7_low_price_proportion" => [Core.Float64(), Core.Float64(), Core.Float64(), ...],
        #             "sum_7_closing_minus_opening_price_growth_rate" => [Core.Float64(), Core.Float64(), Core.Float64(), ...],
        #             "sum_7_KLine_Intuitive_Momentum" => [Core.Float64(), Core.Float64(), Core.Float64(), ...],
        #             "sum_10_turnover_volume_growth_rate" => [Core.Float64(), Core.Float64(), Core.Float64(), ...],
        #             "sum_10_opening_price_growth_rate" => [Core.Float64(), Core.Float64(), Core.Float64(), ...],
        #             "sum_10_closing_price_growth_rate" => [Core.Float64(), Core.Float64(), Core.Float64(), ...],
        #             "sum_10_high_price_proportion" => [Core.Float64(), Core.Float64(), Core.Float64(), ...],
        #             "sum_10_low_price_proportion" => [Core.Float64(), Core.Float64(), Core.Float64(), ...],
        #             "sum_10_closing_minus_opening_price_growth_rate" => [Core.Float64(), Core.Float64(), Core.Float64(), ...],
        #             "sum_10_KLine_Intuitive_Momentum" => [Core.Float64(), Core.Float64(), Core.Float64(), ...],
        #             "average_5_closing_price" => [Core.Float64(0.0), Core.Float64(0.0), Core.Float64(0.0), Core.Float64(0.0), Core.Float64(4.0), Core.Float64(4.084), Core.Float64(4.124), Core.Float64(4.154), Core.Float64(4.206), Core.Float64(4.22), Core.Float64(4.28), ...],
        #             "average_10_closing_price" => [Core.Float64(0.0), Core.Float64(0.0), Core.Float64(0.0), Core.Float64(0.0), Core.Float64(0.0), Core.Float64(0.0), Core.Float64(0.0), Core.Float64(0.0), Core.Float64(0.0), Core.Float64(4.11), Core.Float64(4.182), ...],
        #             "average_20_closing_price" => [Core.Float64(), Core.Float64(), Core.Float64(), ...],
        #             "average_30_closing_price" => [Core.Float64(), Core.Float64(), Core.Float64(), ...],
        #             "Pdata_0" => [Core.Float64(), Core.Float64(), Core.Float64(), Core.Float64()],
        #             "Pupper" => [+Base.Inf, +Base.Inf, +Base.Inf, +Base.Inf],
        #             "Plower" => [-Base.Inf, -Base.Inf, -Base.Inf, -Base.Inf],
        #             "weight" => [Core.Float64(), Core.Float64(), Core.Float64(), ...]
        #         ),
        #         ...
        #     ),
    ...
    # elseif request_Path === "/MarketTiming"
    #     # 客戶端或瀏覽器請求 url = http://[::1]:10001/MarketTiming?Key=username:password&algorithmUser=username&algorithmPass=password&algorithmName=MarketTiming&trading_direction=Long_Position_and_Short_Selling&ticker_symbol=["all"]&is_Optimize=false&MarketTiming_Pdata_0=[3,+0.1,-0.1,0.0]&MarketTiming_Plower=["-Infinity","-Infinity","-Infinity","-Infinity"]&MarketTiming_Pupper=["+Infinity","+Infinity","+Infinity","+Infinity"]&MarketTiming_weight=[]&Cleaned_K_Line=C:/StatisticalServer/Data/steppingData.jld&training_data_file=C:/StatisticalServer/Data/trainingData.jld&testing_data_file=C:/StatisticalServer/Data/testingData.jld&stepping_data_file=C:/StatisticalServer/Data/steppingData.jld
    #     # 客戶端或瀏覽器請求 url = http://127.0.0.1:10001/MarketTiming?Key=username:password&algorithmUser=username&algorithmPass=password&algorithmName=MarketTiming&trading_direction=Long_Position_and_Short_Selling&ticker_symbol=["all"]&is_Optimize=false&MarketTiming_Pdata_0=[3,+0.1,-0.1,0.0]&MarketTiming_Plower=["-Infinity","-Infinity","-Infinity","-Infinity"]&MarketTiming_Pupper=["+Infinity","+Infinity","+Infinity","+Infinity"]&MarketTiming_weight=[]&Cleaned_K_Line=C:/StatisticalServer/Data/steppingData.jld&training_data_file=C:/StatisticalServer/Data/trainingData.jld&testing_data_file=C:/StatisticalServer/Data/testingData.jld&stepping_data_file=C:/StatisticalServer/Data/steppingData.jld

    elseif request_Path === "/PickStock"
        ...
        PickStock_Pdata_0 = [Core.Int64(3), Core.Int64(5)];
        PickStock_Plower = [-Base.Inf, -Base.Inf];
        PickStock_Pupper = [+Base.Inf, +Base.Inf];
        PickStock_weight = [];
        MarketTiming_Pdata_0 = [Core.Int64(3), Core.Float64(+0.1), Core.Float64(-0.1), Core.Float64(0.0)];
        MarketTiming_Plower = [-Base.Inf, -Base.Inf, -Base.Inf, -Base.Inf];
        MarketTiming_Pupper = [+Base.Inf, +Base.Inf, +Base.Inf, +Base.Inf];
        MarketTiming_weight = [];
        training_data = Base.Dict{Core.String, Core.Any}();
        # training_data = JLD.load(Cleaned_K_Line_Daily_file_dir, "training_data");
        # println(training_data);
        testing_data = Base.Dict{Core.String, Core.Any}();
        # testing_data = JLD.load(Cleaned_K_Line_Daily_file_dir, "testing_data");
        # println(testing_data);
        stepping_data = Base.Dict{Core.String, Core.Any}();
        # stepping_data = JLD.load(Cleaned_K_Line_Daily_file_dir, "stepping_data");
        ...
        if Base.haskey(request_data_Dict, "trading_direction")
            if Base.length(request_data_Dict["trading_direction"]) > 0
                investment_method = Base.string(request_data_Dict["trading_direction"][1]);
            end
        end
        # println("Post trading_direction = ", investment_method);
        if Base.haskey(request_Url_Query_Dict, "trading_direction")
            investment_method = Base.string(request_Url_Query_Dict["trading_direction"]);
        end
        # println("URL query trading_direction = ", investment_method);

        if Base.haskey(request_data_Dict, "ticker_symbol")
            if Base.typeof(request_data_Dict["ticker_symbol"]) <: Core.Array || Base.typeof(request_data_Dict["ticker_symbol"]) <: Base.Vector
                ticker_symbol_Array = request_data_Dict["ticker_symbol"];
            end
        end
        # println("Post ticker_symbol :\n", ticker_symbol_Array);
        if Base.haskey(request_Url_Query_Dict, "ticker_symbol")
            code_text_Expr = Base.Meta.parse(request_Url_Query_Dict["ticker_symbol"]);  # 先使用 Base.Meta.parse() 函數解析字符串為代碼;
            ticker_symbol_Array_Source = Base.MainInclude.eval(code_text_Expr);  # 然後再使用 Base.MainInclude.eval() 函數執行字符串代碼語句;
            if Base.typeof(ticker_symbol_Array_Source) <: Core.Array || Base.typeof(ticker_symbol_Array_Source) <: Base.Vector
                ticker_symbol_Array = ticker_symbol_Array_Source;
            end
        end
        # println("URL query ticker_symbol :\n", ticker_symbol_Array);

        if Base.haskey(request_data_Dict, "is_Optimize")
            if Base.length(request_data_Dict["is_Optimize"]) > 0
                # is_Optimize = Base.string(request_data_Dict["is_Optimize"][1]);
                # is_Optimize = Base.parse(Core.Bool, Base.string(request_data_Dict["is_Optimize"][1]));
                if request_data_Dict["is_Optimize"][1] === "true" || request_data_Dict["is_Optimize"][1] === "True" || request_data_Dict["is_Optimize"][1] === "TRUE" || request_data_Dict["is_Optimize"][1] === "1"
                    is_Optimize = true;
                    # is_Optimize = "true";
                end
                if request_data_Dict["is_Optimize"][1] === "false" || request_data_Dict["is_Optimize"][1] === "False" || request_data_Dict["is_Optimize"][1] === "FALSE" || request_data_Dict["is_Optimize"][1] === "0"
                    is_Optimize = false;
                    # is_Optimize = "false";
                end
            end
        end
        # println("Post is_Optimize = ", is_Optimize);
        if Base.haskey(request_Url_Query_Dict, "is_Optimize")
            # is_Optimize = Base.string(request_Url_Query_Dict["is_Optimize"]);
            # is_Optimize = Base.parse(Core.Bool, Base.string(request_Url_Query_Dict["is_Optimize"]));
            if request_Url_Query_Dict["is_Optimize"] === "true" || request_Url_Query_Dict["is_Optimize"] === "True" || request_Url_Query_Dict["is_Optimize"] === "TRUE" || request_Url_Query_Dict["is_Optimize"] === "1"
                is_Optimize = true;
                # is_Optimize = "true";
            end
            if request_Url_Query_Dict["is_Optimize"] === "false" || request_Url_Query_Dict["is_Optimize"] === "False" || request_Url_Query_Dict["is_Optimize"] === "FALSE" || request_Url_Query_Dict["is_Optimize"] === "0"
                is_Optimize = false;
                # is_Optimize = "false";
            end
        end
        # println("URL query is_Optimize = ", is_Optimize);

        if Base.haskey(request_data_Dict, "PickStock_Pdata_0")
            # PickStock_Pdata_0 = request_data_Dict["PickStock_Pdata_0"];
            if (Base.typeof(request_data_Dict["PickStock_Pdata_0"]) <: Core.Array || Base.typeof(request_data_Dict["PickStock_Pdata_0"]) <: Base.Vector) && Base.length(request_data_Dict["PickStock_Pdata_0"]) > 0
                PickStock_Pdata_0 = Core.Array{Core.Float64, 1}();
                for i = 1:Base.length(request_data_Dict["PickStock_Pdata_0"])
                    if Base.isa(request_data_Dict["PickStock_Pdata_0"][i], Core.String) && (request_data_Dict["PickStock_Pdata_0"][i] === "+Base.Inf" || request_data_Dict["PickStock_Pdata_0"][i] === "+Inf" || request_data_Dict["PickStock_Pdata_0"][i] === "+inf" || request_data_Dict["PickStock_Pdata_0"][i] === "+Infinity" || request_data_Dict["PickStock_Pdata_0"][i] === "+infinity" || request_data_Dict["PickStock_Pdata_0"][i] === "Base.Inf" || request_data_Dict["PickStock_Pdata_0"][i] === "Inf" || request_data_Dict["PickStock_Pdata_0"][i] === "inf" || request_data_Dict["PickStock_Pdata_0"][i] === "Infinity" || request_data_Dict["PickStock_Pdata_0"][i] === "infinity")
                        Base.push!(PickStock_Pdata_0, +Base.Inf);
                    elseif Base.isa(request_data_Dict["PickStock_Pdata_0"][i], Core.String) && (request_data_Dict["PickStock_Pdata_0"][i] === "-Base.Inf" || request_data_Dict["PickStock_Pdata_0"][i] === "-Inf" || request_data_Dict["PickStock_Pdata_0"][i] === "-inf" || request_data_Dict["PickStock_Pdata_0"][i] === "-Infinity" || request_data_Dict["PickStock_Pdata_0"][i] === "-infinity")
                        Base.push!(PickStock_Pdata_0, -Base.Inf);
                    else
                        Base.push!(PickStock_Pdata_0, Core.Float64(request_data_Dict["PickStock_Pdata_0"][i]));
                    end
                end
            end
        end
        # println("Post PickStock Pdata_0 :\n", PickStock_Pdata_0);
        if Base.haskey(request_Url_Query_Dict, "PickStock_Pdata_0")
            code_text_Expr = Base.Meta.parse(request_Url_Query_Dict["PickStock_Pdata_0"]);  # 先使用 Base.Meta.parse() 函數解析字符串為代碼;
            PickStock_Pdata_0_Source = Base.MainInclude.eval(code_text_Expr);  # 然後再使用 Base.MainInclude.eval() 函數執行字符串代碼語句;
            # PickStock_Pdata_0 = PickStock_Pdata_0_Source;
            if (Base.typeof(PickStock_Pdata_0_Source) <: Core.Array || Base.typeof(PickStock_Pdata_0_Source) <: Base.Vector) && Base.length(PickStock_Pdata_0_Source) > 0
                PickStock_Pdata_0 = Core.Array{Core.Float64, 1}();
                for i = 1:Base.length(PickStock_Pdata_0_Source)
                    if Base.isa(PickStock_Pdata_0_Source[i], Core.String) && (PickStock_Pdata_0_Source[i] === "+Base.Inf" || PickStock_Pdata_0_Source[i] === "+Inf" || PickStock_Pdata_0_Source[i] === "+inf" || PickStock_Pdata_0_Source[i] === "+Infinity" || PickStock_Pdata_0_Source[i] === "+infinity" || PickStock_Pdata_0_Source[i] === "Base.Inf" || PickStock_Pdata_0_Source[i] === "Inf" || PickStock_Pdata_0_Source[i] === "inf" || PickStock_Pdata_0_Source[i] === "Infinity" || PickStock_Pdata_0_Source[i] === "infinity")
                        Base.push!(PickStock_Pdata_0, +Base.Inf);
                    elseif Base.isa(PickStock_Pdata_0_Source[i], Core.String) && (PickStock_Pdata_0_Source[i] === "-Base.Inf" || PickStock_Pdata_0_Source[i] === "-Inf" || PickStock_Pdata_0_Source[i] === "-inf" || PickStock_Pdata_0_Source[i] === "-Infinity" || PickStock_Pdata_0_Source[i] === "-infinity")
                        Base.push!(PickStock_Pdata_0, -Base.Inf);
                    else
                        Base.push!(PickStock_Pdata_0, Core.Float64(PickStock_Pdata_0_Source[i]));
                    end
                end
            end
        end
        # println("URL query PickStock Pdata_0 :\n", PickStock_Pdata_0);

        if Base.haskey(request_data_Dict, "PickStock_Plower")
            # PickStock_Plower = request_data_Dict["PickStock_Plower"];
            if (Base.typeof(request_data_Dict["PickStock_Plower"]) <: Core.Array || Base.typeof(request_data_Dict["PickStock_Plower"]) <: Base.Vector) && Base.length(request_data_Dict["PickStock_Plower"]) > 0
                PickStock_Plower = Core.Array{Core.Float64, 1}();
                for i = 1:Base.length(request_data_Dict["PickStock_Plower"])
                    if Base.isa(request_data_Dict["PickStock_Plower"][i], Core.String) && (request_data_Dict["PickStock_Plower"][i] === "+Base.Inf" || request_data_Dict["PickStock_Plower"][i] === "+Inf" || request_data_Dict["PickStock_Plower"][i] === "+inf" || request_data_Dict["PickStock_Plower"][i] === "+Infinity" || request_data_Dict["PickStock_Plower"][i] === "+infinity" || request_data_Dict["PickStock_Plower"][i] === "Base.Inf" || request_data_Dict["PickStock_Plower"][i] === "Inf" || request_data_Dict["PickStock_Plower"][i] === "inf" || request_data_Dict["PickStock_Plower"][i] === "Infinity" || request_data_Dict["PickStock_Plower"][i] === "infinity")
                        Base.push!(PickStock_Plower, +Base.Inf);
                    elseif Base.isa(request_data_Dict["PickStock_Plower"][i], Core.String) && (request_data_Dict["PickStock_Plower"][i] === "-Base.Inf" || request_data_Dict["PickStock_Plower"][i] === "-Inf" || request_data_Dict["PickStock_Plower"][i] === "-inf" || request_data_Dict["PickStock_Plower"][i] === "-Infinity" || request_data_Dict["PickStock_Plower"][i] === "-infinity")
                        Base.push!(PickStock_Plower, -Base.Inf);
                    else
                        Base.push!(PickStock_Plower, Core.Float64(request_data_Dict["PickStock_Plower"][i]));
                    end
                    # if request_data_Dict["PickStock_Plower"][i] === "Inf" || request_data_Dict["PickStock_Plower"][i] === "-Inf" || request_data_Dict["PickStock_Plower"][i] === "+Inf"
                    #     try
                    #         codeText = Base.Meta.parse(request_data_Dict["PickStock_Plower"][i]);  # 先使用 Base.Meta.parse() 函數解析字符串為代碼;
                    #         Plower_Value = Base.MainInclude.eval(codeText);  # 然後再使用 Base.MainInclude.eval() 函數執行字符串代碼語句;
                    #         Base.push!(PickStock_Plower, Plower_Value);
                    #     catch err
                    #         println(err);
                    #         Base.push!(PickStock_Plower, err);
                    #     end
                    # else
                    #     Base.push!(PickStock_Plower, Core.Float64(request_data_Dict["PickStock_Plower"][i]));
                    # end
        ...
        # println("Post PickStock Plower :\n", PickStock_Plower);
        if Base.haskey(request_Url_Query_Dict, "PickStock_Plower")
            code_text_Expr = Base.Meta.parse(request_Url_Query_Dict["PickStock_Plower"]);  # 先使用 Base.Meta.parse() 函數解析字符串為代碼;
            PickStock_Plower_Source = Base.MainInclude.eval(code_text_Expr);  # 然後再使用 Base.MainInclude.eval() 函數執行字符串代碼語句;
            # PickStock_Plower = PickStock_Plower_Source;
            if (Base.typeof(PickStock_Plower_Source) <: Core.Array || Base.typeof(PickStock_Plower_Source) <: Base.Vector) && Base.length(PickStock_Plower_Source) > 0
                PickStock_Plower = Core.Array{Core.Float64, 1}();
                for i = 1:Base.length(PickStock_Plower_Source)
                    if Base.isa(PickStock_Plower_Source[i], Core.String) && (PickStock_Plower_Source[i] === "+Base.Inf" || PickStock_Plower_Source[i] === "+Inf" || PickStock_Plower_Source[i] === "+inf" || PickStock_Plower_Source[i] === "+Infinity" || PickStock_Plower_Source[i] === "+infinity" || PickStock_Plower_Source[i] === "Base.Inf" || PickStock_Plower_Source[i] === "Inf" || PickStock_Plower_Source[i] === "inf" || PickStock_Plower_Source[i] === "Infinity" || PickStock_Plower_Source[i] === "infinity")
                        Base.push!(PickStock_Plower, +Base.Inf);
                    elseif Base.isa(PickStock_Plower_Source[i], Core.String) && (PickStock_Plower_Source[i] === "-Base.Inf" || PickStock_Plower_Source[i] === "-Inf" || PickStock_Plower_Source[i] === "-inf" || PickStock_Plower_Source[i] === "-Infinity" || PickStock_Plower_Source[i] === "-infinity")
                        Base.push!(PickStock_Plower, -Base.Inf);
                    else
                        Base.push!(PickStock_Plower, Core.Float64(PickStock_Plower_Source[i]));
                    end
                end
            end
        end
        # println("URL query PickStock Plower :\n", PickStock_Plower);

        if Base.haskey(request_data_Dict, "PickStock_Pupper")
            # PickStock_Pupper = request_data_Dict["PickStock_Pupper"];
            if (Base.typeof(request_data_Dict["PickStock_Pupper"]) <: Core.Array || Base.typeof(request_data_Dict["PickStock_Pupper"]) <: Base.Vector) && Base.length(request_data_Dict["PickStock_Pupper"]) > 0
                PickStock_Pupper = Core.Array{Core.Float64, 1}();
                for i = 1:Base.length(request_data_Dict["PickStock_Pupper"])
                    if Base.isa(request_data_Dict["PickStock_Pupper"][i], Core.String) && (request_data_Dict["PickStock_Pupper"][i] === "+Base.Inf" || request_data_Dict["PickStock_Pupper"][i] === "+Inf" || request_data_Dict["PickStock_Pupper"][i] === "+inf" || request_data_Dict["PickStock_Pupper"][i] === "+Infinity" || request_data_Dict["PickStock_Pupper"][i] === "+infinity" || request_data_Dict["PickStock_Pupper"][i] === "Base.Inf" || request_data_Dict["PickStock_Pupper"][i] === "Inf" || request_data_Dict["PickStock_Pupper"][i] === "inf" || request_data_Dict["PickStock_Pupper"][i] === "Infinity" || request_data_Dict["PickStock_Pupper"][i] === "infinity")
                        Base.push!(PickStock_Pupper, +Base.Inf);
                    elseif Base.isa(request_data_Dict["PickStock_Pupper"][i], Core.String) && (request_data_Dict["PickStock_Pupper"][i] === "-Base.Inf" || request_data_Dict["PickStock_Pupper"][i] === "-Inf" || request_data_Dict["PickStock_Pupper"][i] === "-inf" || request_data_Dict["PickStock_Pupper"][i] === "-Infinity" || request_data_Dict["PickStock_Pupper"][i] === "-infinity")
                        Base.push!(PickStock_Pupper, -Base.Inf);
                    else
                        Base.push!(PickStock_Pupper, Core.Float64(request_data_Dict["PickStock_Pupper"][i]));
                    end
                    # if request_data_Dict["PickStock_Pupper"][i] === "Inf" || request_data_Dict["PickStock_Pupper"][i] === "-Inf" || request_data_Dict["PickStock_Pupper"][i] === "+Inf"
                    #     try
                    #         codeText = Base.Meta.parse(request_data_Dict["PickStock_Pupper"][i]);  # 先使用 Base.Meta.parse() 函數解析字符串為代碼;
                    #         Plower_Value = Base.MainInclude.eval(codeText);  # 然後再使用 Base.MainInclude.eval() 函數執行字符串代碼語句;
                    #         Base.push!(PickStock_Pupper, Plower_Value);
                    #     catch err
                    #         println(err);
                    #         Base.push!(PickStock_Pupper, err);
                    #     end
                    # else
                    #     Base.push!(PickStock_Pupper, Core.Float64(request_data_Dict["PickStock_Pupper"][i]));
                    # end
        ...
        # println("Post PickStock Pupper :\n", PickStock_Pupper);
        if Base.haskey(request_Url_Query_Dict, "PickStock_Pupper")
            code_text_Expr = Base.Meta.parse(request_Url_Query_Dict["PickStock_Pupper"]);  # 先使用 Base.Meta.parse() 函數解析字符串為代碼;
            PickStock_Pupper_Source = Base.MainInclude.eval(code_text_Expr);  # 然後再使用 Base.MainInclude.eval() 函數執行字符串代碼語句;
            # PickStock_Pupper = PickStock_Pupper_Source;
            if (Base.typeof(PickStock_Pupper_Source) <: Core.Array || Base.typeof(PickStock_Pupper_Source) <: Base.Vector) && Base.length(PickStock_Pupper_Source) > 0
                PickStock_Pupper = Core.Array{Core.Float64, 1}();
                for i = 1:Base.length(PickStock_Pupper_Source)
                    if Base.isa(PickStock_Pupper_Source[i], Core.String) && (PickStock_Pupper_Source[i] === "+Base.Inf" || PickStock_Pupper_Source[i] === "+Inf" || PickStock_Pupper_Source[i] === "+inf" || PickStock_Pupper_Source[i] === "+Infinity" || PickStock_Pupper_Source[i] === "+infinity" || PickStock_Pupper_Source[i] === "Base.Inf" || PickStock_Pupper_Source[i] === "Inf" || PickStock_Pupper_Source[i] === "inf" || PickStock_Pupper_Source[i] === "Infinity" || PickStock_Pupper_Source[i] === "infinity")
                        Base.push!(PickStock_Pupper, +Base.Inf);
                    elseif Base.isa(PickStock_Pupper_Source[i], Core.String) && (PickStock_Pupper_Source[i] === "-Base.Inf" || PickStock_Pupper_Source[i] === "-Inf" || PickStock_Pupper_Source[i] === "-inf" || PickStock_Pupper_Source[i] === "-Infinity" || PickStock_Pupper_Source[i] === "-infinity")
                        Base.push!(PickStock_Pupper, -Base.Inf);
                    else
                        Base.push!(PickStock_Pupper, Core.Float64(PickStock_Pupper_Source[i]));
                    end
                end
            end
        end
        # println("URL query PickStock Pupper :\n", PickStock_Pupper);

        if Base.haskey(request_data_Dict, "PickStock_weight")
            # PickStock_weight = request_data_Dict["PickStock_weight"];
            if (Base.typeof(request_data_Dict["PickStock_weight"]) <: Core.Array || Base.typeof(request_data_Dict["PickStock_weight"]) <: Base.Vector) && Base.length(request_data_Dict["PickStock_weight"]) > 0
                PickStock_weight = Core.Array{Core.Float64, 1}();
                for i = 1:Base.length(request_data_Dict["PickStock_weight"])
                    if Base.isa(request_data_Dict["PickStock_weight"][i], Core.String) && (request_data_Dict["PickStock_weight"][i] === "+Base.Inf" || request_data_Dict["PickStock_weight"][i] === "+Inf" || request_data_Dict["PickStock_weight"][i] === "+inf" || request_data_Dict["PickStock_weight"][i] === "+Infinity" || request_data_Dict["PickStock_weight"][i] === "+infinity" || request_data_Dict["PickStock_weight"][i] === "Base.Inf" || request_data_Dict["PickStock_weight"][i] === "Inf" || request_data_Dict["PickStock_weight"][i] === "inf" || request_data_Dict["PickStock_weight"][i] === "Infinity" || request_data_Dict["PickStock_weight"][i] === "infinity")
                        Base.push!(PickStock_weight, +Base.Inf);
                    elseif Base.isa(request_data_Dict["PickStock_weight"][i], Core.String) && (request_data_Dict["PickStock_weight"][i] === "-Base.Inf" || request_data_Dict["PickStock_weight"][i] === "-Inf" || request_data_Dict["PickStock_weight"][i] === "-inf" || request_data_Dict["PickStock_weight"][i] === "-Infinity" || request_data_Dict["PickStock_weight"][i] === "-infinity")
                        Base.push!(PickStock_weight, -Base.Inf);
                    else
                        Base.push!(PickStock_weight, Core.Float64(request_data_Dict["PickStock_weight"][i]));
                    end
                    # if request_data_Dict["PickStock_weight"][i] === "Inf" || request_data_Dict["PickStock_weight"][i] === "-Inf" || request_data_Dict["PickStock_weight"][i] === "+Inf"
                    #     try
                    #         codeText = Base.Meta.parse(request_data_Dict["PickStock_weight"][i]);  # 先使用 Base.Meta.parse() 函數解析字符串為代碼;
                    #         Plower_Value = Base.MainInclude.eval(codeText);  # 然後再使用 Base.MainInclude.eval() 函數執行字符串代碼語句;
                    #         Base.push!(PickStock_weight, Plower_Value);
                    #     catch err
                    #         println(err);
                    #         Base.push!(PickStock_weight, err);
                    #     end
                    # else
                    #     Base.push!(PickStock_weight, Core.Float64(request_data_Dict["PickStock_weight"][i]));
                    # end
        ...
        # println("Post PickStock weight :\n", PickStock_weight);
        if Base.haskey(request_Url_Query_Dict, "PickStock_weight")
            code_text_Expr = Base.Meta.parse(request_Url_Query_Dict["PickStock_weight"]);  # 先使用 Base.Meta.parse() 函數解析字符串為代碼;
            PickStock_weight_Source = Base.MainInclude.eval(code_text_Expr);  # 然後再使用 Base.MainInclude.eval() 函數執行字符串代碼語句;
            # PickStock_weight = PickStock_weight_Source;
            if (Base.typeof(PickStock_weight_Source) <: Core.Array || Base.typeof(PickStock_weight_Source) <: Base.Vector) && Base.length(PickStock_weight_Source) > 0
                PickStock_weight = Core.Array{Core.Float64, 1}();
                for i = 1:Base.length(PickStock_weight_Source)
                    if Base.isa(PickStock_weight_Source[i], Core.String) && (PickStock_weight_Source[i] === "+Base.Inf" || PickStock_weight_Source[i] === "+Inf" || PickStock_weight_Source[i] === "+inf" || PickStock_weight_Source[i] === "+Infinity" || PickStock_weight_Source[i] === "+infinity" || PickStock_weight_Source[i] === "Base.Inf" || PickStock_weight_Source[i] === "Inf" || PickStock_weight_Source[i] === "inf" || PickStock_weight_Source[i] === "Infinity" || PickStock_weight_Source[i] === "infinity")
                        Base.push!(PickStock_weight, +Base.Inf);
                    elseif Base.isa(PickStock_weight_Source[i], Core.String) && (PickStock_weight_Source[i] === "-Base.Inf" || PickStock_weight_Source[i] === "-Inf" || PickStock_weight_Source[i] === "-inf" || PickStock_weight_Source[i] === "-Infinity" || PickStock_weight_Source[i] === "-infinity")
                        Base.push!(PickStock_weight, -Base.Inf);
                    else
                        Base.push!(PickStock_weight, Core.Float64(PickStock_weight_Source[i]));
                    end
                end
            end
        end
        # println("URL query PickStock weight :\n", PickStock_weight);

        if Base.haskey(request_data_Dict, "MarketTiming_Pdata_0")
            # MarketTiming_Pdata_0 = request_data_Dict["MarketTiming_Pdata_0"];
            if (Base.typeof(request_data_Dict["MarketTiming_Pdata_0"]) <: Core.Array || Base.typeof(request_data_Dict["MarketTiming_Pdata_0"]) <: Base.Vector) && Base.length(request_data_Dict["MarketTiming_Pdata_0"]) > 0
                MarketTiming_Pdata_0 = Core.Array{Core.Float64, 1}();
                for i = 1:Base.length(request_data_Dict["MarketTiming_Pdata_0"])
                    if Base.isa(request_data_Dict["MarketTiming_Pdata_0"][i], Core.String) && (request_data_Dict["MarketTiming_Pdata_0"][i] === "+Base.Inf" || request_data_Dict["MarketTiming_Pdata_0"][i] === "+Inf" || request_data_Dict["MarketTiming_Pdata_0"][i] === "+inf" || request_data_Dict["MarketTiming_Pdata_0"][i] === "+Infinity" || request_data_Dict["MarketTiming_Pdata_0"][i] === "+infinity" || request_data_Dict["MarketTiming_Pdata_0"][i] === "Base.Inf" || request_data_Dict["MarketTiming_Pdata_0"][i] === "Inf" || request_data_Dict["MarketTiming_Pdata_0"][i] === "inf" || request_data_Dict["MarketTiming_Pdata_0"][i] === "Infinity" || request_data_Dict["MarketTiming_Pdata_0"][i] === "infinity")
                        Base.push!(MarketTiming_Pdata_0, +Base.Inf);
                    elseif Base.isa(request_data_Dict["MarketTiming_Pdata_0"][i], Core.String) && (request_data_Dict["MarketTiming_Pdata_0"][i] === "-Base.Inf" || request_data_Dict["MarketTiming_Pdata_0"][i] === "-Inf" || request_data_Dict["MarketTiming_Pdata_0"][i] === "-inf" || request_data_Dict["MarketTiming_Pdata_0"][i] === "-Infinity" || request_data_Dict["MarketTiming_Pdata_0"][i] === "-infinity")
                        Base.push!(MarketTiming_Pdata_0, -Base.Inf);
                    else
                        Base.push!(MarketTiming_Pdata_0, Core.Float64(request_data_Dict["MarketTiming_Pdata_0"][i]));
                    end
                end
            end
        end
        # println("Post MarketTiming Pdata_0 :\n", MarketTiming_Pdata_0);
        if Base.haskey(request_Url_Query_Dict, "MarketTiming_Pdata_0")
            code_text_Expr = Base.Meta.parse(request_Url_Query_Dict["MarketTiming_Pdata_0"]);  # 先使用 Base.Meta.parse() 函數解析字符串為代碼;
            MarketTiming_Pdata_0_Source = Base.MainInclude.eval(code_text_Expr);  # 然後再使用 Base.MainInclude.eval() 函數執行字符串代碼語句;
            # MarketTiming_Pdata_0 = MarketTiming_Pdata_0_Source;
            if (Base.typeof(MarketTiming_Pdata_0_Source) <: Core.Array || Base.typeof(MarketTiming_Pdata_0_Source) <: Base.Vector) && Base.length(MarketTiming_Pdata_0_Source) > 0
                MarketTiming_Pdata_0 = Core.Array{Core.Float64, 1}();
                for i = 1:Base.length(MarketTiming_Pdata_0_Source)
                    if Base.isa(MarketTiming_Pdata_0_Source[i], Core.String) && (MarketTiming_Pdata_0_Source[i] === "+Base.Inf" || MarketTiming_Pdata_0_Source[i] === "+Inf" || MarketTiming_Pdata_0_Source[i] === "+inf" || MarketTiming_Pdata_0_Source[i] === "+Infinity" || MarketTiming_Pdata_0_Source[i] === "+infinity" || MarketTiming_Pdata_0_Source[i] === "Base.Inf" || MarketTiming_Pdata_0_Source[i] === "Inf" || MarketTiming_Pdata_0_Source[i] === "inf" || MarketTiming_Pdata_0_Source[i] === "Infinity" || MarketTiming_Pdata_0_Source[i] === "infinity")
                        Base.push!(MarketTiming_Pdata_0, +Base.Inf);
                    elseif Base.isa(MarketTiming_Pdata_0_Source[i], Core.String) && (MarketTiming_Pdata_0_Source[i] === "-Base.Inf" || MarketTiming_Pdata_0_Source[i] === "-Inf" || MarketTiming_Pdata_0_Source[i] === "-inf" || MarketTiming_Pdata_0_Source[i] === "-Infinity" || MarketTiming_Pdata_0_Source[i] === "-infinity")
                        Base.push!(MarketTiming_Pdata_0, -Base.Inf);
                    else
                        Base.push!(MarketTiming_Pdata_0, Core.Float64(MarketTiming_Pdata_0_Source[i]));
                    end
                end
            end
        end
        # println("URL query MarketTiming Pdata_0 :\n", MarketTiming_Pdata_0);

        if Base.haskey(request_data_Dict, "MarketTiming_Plower")
            # MarketTiming_Plower = request_data_Dict["MarketTiming_Plower"];
            if (Base.typeof(request_data_Dict["MarketTiming_Plower"]) <: Core.Array || Base.typeof(request_data_Dict["MarketTiming_Plower"]) <: Base.Vector) && Base.length(request_data_Dict["MarketTiming_Plower"]) > 0
                MarketTiming_Plower = Core.Array{Core.Float64, 1}();
                for i = 1:Base.length(request_data_Dict["MarketTiming_Plower"])
                    if Base.isa(request_data_Dict["MarketTiming_Plower"][i], Core.String) && (request_data_Dict["MarketTiming_Plower"][i] === "+Base.Inf" || request_data_Dict["MarketTiming_Plower"][i] === "+Inf" || request_data_Dict["MarketTiming_Plower"][i] === "+inf" || request_data_Dict["MarketTiming_Plower"][i] === "+Infinity" || request_data_Dict["MarketTiming_Plower"][i] === "+infinity" || request_data_Dict["MarketTiming_Plower"][i] === "Base.Inf" || request_data_Dict["MarketTiming_Plower"][i] === "Inf" || request_data_Dict["MarketTiming_Plower"][i] === "inf" || request_data_Dict["MarketTiming_Plower"][i] === "Infinity" || request_data_Dict["MarketTiming_Plower"][i] === "infinity")
                        Base.push!(MarketTiming_Plower, +Base.Inf);
                    elseif Base.isa(request_data_Dict["MarketTiming_Plower"][i], Core.String) && (request_data_Dict["MarketTiming_Plower"][i] === "-Base.Inf" || request_data_Dict["MarketTiming_Plower"][i] === "-Inf" || request_data_Dict["MarketTiming_Plower"][i] === "-inf" || request_data_Dict["MarketTiming_Plower"][i] === "-Infinity" || request_data_Dict["MarketTiming_Plower"][i] === "-infinity")
                        Base.push!(MarketTiming_Plower, -Base.Inf);
                    else
                        Base.push!(MarketTiming_Plower, Core.Float64(request_data_Dict["MarketTiming_Plower"][i]));
                    end
                    # if request_data_Dict["MarketTiming_Plower"][i] === "Inf" || request_data_Dict["MarketTiming_Plower"][i] === "-Inf" || request_data_Dict["MarketTiming_Plower"][i] === "+Inf"
                    #     try
                    #         codeText = Base.Meta.parse(request_data_Dict["MarketTiming_Plower"][i]);  # 先使用 Base.Meta.parse() 函數解析字符串為代碼;
                    #         Plower_Value = Base.MainInclude.eval(codeText);  # 然後再使用 Base.MainInclude.eval() 函數執行字符串代碼語句;
                    #         Base.push!(MarketTiming_Plower, Plower_Value);
                    #     catch err
                    #         println(err);
                    #         Base.push!(MarketTiming_Plower, err);
                    #     end
                    # else
                    #     Base.push!(MarketTiming_Plower, Core.Float64(request_data_Dict["MarketTiming_Plower"][i]));
                    # end
        ...
        # println("Post MarketTiming Plower :\n", MarketTiming_Plower);
        if Base.haskey(request_Url_Query_Dict, "MarketTiming_Plower")
            code_text_Expr = Base.Meta.parse(request_Url_Query_Dict["MarketTiming_Plower"]);  # 先使用 Base.Meta.parse() 函數解析字符串為代碼;
            MarketTiming_Plower_Source = Base.MainInclude.eval(code_text_Expr);  # 然後再使用 Base.MainInclude.eval() 函數執行字符串代碼語句;
            # MarketTiming_Plower = MarketTiming_Plower_Source;
            if (Base.typeof(MarketTiming_Plower_Source) <: Core.Array || Base.typeof(MarketTiming_Plower_Source) <: Base.Vector) && Base.length(MarketTiming_Plower_Source) > 0
                MarketTiming_Plower = Core.Array{Core.Float64, 1}();
                for i = 1:Base.length(MarketTiming_Plower_Source)
                    if Base.isa(MarketTiming_Plower_Source[i], Core.String) && (MarketTiming_Plower_Source[i] === "+Base.Inf" || MarketTiming_Plower_Source[i] === "+Inf" || MarketTiming_Plower_Source[i] === "+inf" || MarketTiming_Plower_Source[i] === "+Infinity" || MarketTiming_Plower_Source[i] === "+infinity" || MarketTiming_Plower_Source[i] === "Base.Inf" || MarketTiming_Plower_Source[i] === "Inf" || MarketTiming_Plower_Source[i] === "inf" || MarketTiming_Plower_Source[i] === "Infinity" || MarketTiming_Plower_Source[i] === "infinity")
                        Base.push!(MarketTiming_Plower, +Base.Inf);
                    elseif Base.isa(MarketTiming_Plower_Source[i], Core.String) && (MarketTiming_Plower_Source[i] === "-Base.Inf" || MarketTiming_Plower_Source[i] === "-Inf" || MarketTiming_Plower_Source[i] === "-inf" || MarketTiming_Plower_Source[i] === "-Infinity" || MarketTiming_Plower_Source[i] === "-infinity")
                        Base.push!(MarketTiming_Plower, -Base.Inf);
                    else
                        Base.push!(MarketTiming_Plower, Core.Float64(MarketTiming_Plower_Source[i]));
                    end
                end
            end
        end
        # println("URL query MarketTiming Plower :\n", MarketTiming_Plower);

        if Base.haskey(request_data_Dict, "MarketTiming_Pupper")
            # MarketTiming_Pupper = request_data_Dict["MarketTiming_Pupper"];
            if (Base.typeof(request_data_Dict["MarketTiming_Pupper"]) <: Core.Array || Base.typeof(request_data_Dict["MarketTiming_Pupper"]) <: Base.Vector) && Base.length(request_data_Dict["MarketTiming_Pupper"]) > 0
                MarketTiming_Pupper = Core.Array{Core.Float64, 1}();
                for i = 1:Base.length(request_data_Dict["MarketTiming_Pupper"])
                    if Base.isa(request_data_Dict["MarketTiming_Pupper"][i], Core.String) && (request_data_Dict["MarketTiming_Pupper"][i] === "+Base.Inf" || request_data_Dict["MarketTiming_Pupper"][i] === "+Inf" || request_data_Dict["MarketTiming_Pupper"][i] === "+inf" || request_data_Dict["MarketTiming_Pupper"][i] === "+Infinity" || request_data_Dict["MarketTiming_Pupper"][i] === "+infinity" || request_data_Dict["MarketTiming_Pupper"][i] === "Base.Inf" || request_data_Dict["MarketTiming_Pupper"][i] === "Inf" || request_data_Dict["MarketTiming_Pupper"][i] === "inf" || request_data_Dict["MarketTiming_Pupper"][i] === "Infinity" || request_data_Dict["MarketTiming_Pupper"][i] === "infinity")
                        Base.push!(MarketTiming_Pupper, +Base.Inf);
                    elseif Base.isa(request_data_Dict["MarketTiming_Pupper"][i], Core.String) && (request_data_Dict["MarketTiming_Pupper"][i] === "-Base.Inf" || request_data_Dict["MarketTiming_Pupper"][i] === "-Inf" || request_data_Dict["MarketTiming_Pupper"][i] === "-inf" || request_data_Dict["MarketTiming_Pupper"][i] === "-Infinity" || request_data_Dict["MarketTiming_Pupper"][i] === "-infinity")
                        Base.push!(MarketTiming_Pupper, -Base.Inf);
                    else
                        Base.push!(MarketTiming_Pupper, Core.Float64(request_data_Dict["MarketTiming_Pupper"][i]));
                    end
                    # if request_data_Dict["MarketTiming_Pupper"][i] === "Inf" || request_data_Dict["MarketTiming_Pupper"][i] === "-Inf" || request_data_Dict["MarketTiming_Pupper"][i] === "+Inf"
                    #     try
                    #         codeText = Base.Meta.parse(request_data_Dict["MarketTiming_Pupper"][i]);  # 先使用 Base.Meta.parse() 函數解析字符串為代碼;
                    #         Plower_Value = Base.MainInclude.eval(codeText);  # 然後再使用 Base.MainInclude.eval() 函數執行字符串代碼語句;
                    #         Base.push!(MarketTiming_Pupper, Plower_Value);
                    #     catch err
                    #         println(err);
                    #         Base.push!(MarketTiming_Pupper, err);
                    #     end
                    # else
                    #     Base.push!(MarketTiming_Pupper, Core.Float64(request_data_Dict["MarketTiming_Pupper"][i]));
                    # end
        ...
        # println("Post MarketTiming Pupper :\n", MarketTiming_Pupper);
        if Base.haskey(request_Url_Query_Dict, "MarketTiming_Pupper")
            code_text_Expr = Base.Meta.parse(request_Url_Query_Dict["MarketTiming_Pupper"]);  # 先使用 Base.Meta.parse() 函數解析字符串為代碼;
            MarketTiming_Pupper_Source = Base.MainInclude.eval(code_text_Expr);  # 然後再使用 Base.MainInclude.eval() 函數執行字符串代碼語句;
            # MarketTiming_Pupper = MarketTiming_Pupper_Source;
            if (Base.typeof(MarketTiming_Pupper_Source) <: Core.Array || Base.typeof(MarketTiming_Pupper_Source) <: Base.Vector) && Base.length(MarketTiming_Pupper_Source) > 0
                MarketTiming_Pupper = Core.Array{Core.Float64, 1}();
                for i = 1:Base.length(MarketTiming_Pupper_Source)
                    if Base.isa(MarketTiming_Pupper_Source[i], Core.String) && (MarketTiming_Pupper_Source[i] === "+Base.Inf" || MarketTiming_Pupper_Source[i] === "+Inf" || MarketTiming_Pupper_Source[i] === "+inf" || MarketTiming_Pupper_Source[i] === "+Infinity" || MarketTiming_Pupper_Source[i] === "+infinity" || MarketTiming_Pupper_Source[i] === "Base.Inf" || MarketTiming_Pupper_Source[i] === "Inf" || MarketTiming_Pupper_Source[i] === "inf" || MarketTiming_Pupper_Source[i] === "Infinity" || MarketTiming_Pupper_Source[i] === "infinity")
                        Base.push!(MarketTiming_Pupper, +Base.Inf);
                    elseif Base.isa(MarketTiming_Pupper_Source[i], Core.String) && (MarketTiming_Pupper_Source[i] === "-Base.Inf" || MarketTiming_Pupper_Source[i] === "-Inf" || MarketTiming_Pupper_Source[i] === "-inf" || MarketTiming_Pupper_Source[i] === "-Infinity" || MarketTiming_Pupper_Source[i] === "-infinity")
                        Base.push!(MarketTiming_Pupper, -Base.Inf);
                    else
                        Base.push!(MarketTiming_Pupper, Core.Float64(MarketTiming_Pupper_Source[i]));
                    end
                end
            end
        end
        # println("URL query MarketTiming Pupper :\n", MarketTiming_Pupper);

        if Base.haskey(request_data_Dict, "MarketTiming_weight")
            # MarketTiming_weight = request_data_Dict["MarketTiming_weight"];
            if (Base.typeof(request_data_Dict["MarketTiming_weight"]) <: Core.Array || Base.typeof(request_data_Dict["MarketTiming_weight"]) <: Base.Vector) && Base.length(request_data_Dict["MarketTiming_weight"]) > 0
                MarketTiming_weight = Core.Array{Core.Float64, 1}();
                for i = 1:Base.length(request_data_Dict["MarketTiming_weight"])
                    if Base.isa(request_data_Dict["MarketTiming_weight"][i], Core.String) && (request_data_Dict["MarketTiming_weight"][i] === "+Base.Inf" || request_data_Dict["MarketTiming_weight"][i] === "+Inf" || request_data_Dict["MarketTiming_weight"][i] === "+inf" || request_data_Dict["MarketTiming_weight"][i] === "+Infinity" || request_data_Dict["MarketTiming_weight"][i] === "+infinity" || request_data_Dict["MarketTiming_weight"][i] === "Base.Inf" || request_data_Dict["MarketTiming_weight"][i] === "Inf" || request_data_Dict["MarketTiming_weight"][i] === "inf" || request_data_Dict["MarketTiming_weight"][i] === "Infinity" || request_data_Dict["MarketTiming_weight"][i] === "infinity")
                        Base.push!(MarketTiming_weight, +Base.Inf);
                    elseif Base.isa(request_data_Dict["MarketTiming_weight"][i], Core.String) && (request_data_Dict["MarketTiming_weight"][i] === "-Base.Inf" || request_data_Dict["MarketTiming_weight"][i] === "-Inf" || request_data_Dict["MarketTiming_weight"][i] === "-inf" || request_data_Dict["MarketTiming_weight"][i] === "-Infinity" || request_data_Dict["MarketTiming_weight"][i] === "-infinity")
                        Base.push!(MarketTiming_weight, -Base.Inf);
                    else
                        Base.push!(MarketTiming_weight, Core.Float64(request_data_Dict["MarketTiming_weight"][i]));
                    end
                    # if request_data_Dict["MarketTiming_weight"][i] === "Inf" || request_data_Dict["MarketTiming_weight"][i] === "-Inf" || request_data_Dict["MarketTiming_weight"][i] === "+Inf"
                    #     try
                    #         codeText = Base.Meta.parse(request_data_Dict["MarketTiming_weight"][i]);  # 先使用 Base.Meta.parse() 函數解析字符串為代碼;
                    #         Plower_Value = Base.MainInclude.eval(codeText);  # 然後再使用 Base.MainInclude.eval() 函數執行字符串代碼語句;
                    #         Base.push!(MarketTiming_weight, Plower_Value);
                    #     catch err
                    #         println(err);
                    #         Base.push!(MarketTiming_weight, err);
                    #     end
                    # else
                    #     Base.push!(MarketTiming_weight, Core.Float64(request_data_Dict["MarketTiming_weight"][i]));
                    # end
        ...
        # println("Post MarketTiming weight :\n", MarketTiming_weight);
        if Base.haskey(request_Url_Query_Dict, "MarketTiming_weight")
            code_text_Expr = Base.Meta.parse(request_Url_Query_Dict["MarketTiming_weight"]);  # 先使用 Base.Meta.parse() 函數解析字符串為代碼;
            MarketTiming_weight_Source = Base.MainInclude.eval(code_text_Expr);  # 然後再使用 Base.MainInclude.eval() 函數執行字符串代碼語句;
            # MarketTiming_weight = MarketTiming_weight_Source;
            if (Base.typeof(MarketTiming_weight_Source) <: Core.Array || Base.typeof(MarketTiming_weight_Source) <: Base.Vector) && Base.length(MarketTiming_weight_Source) > 0
                MarketTiming_weight = Core.Array{Core.Float64, 1}();
                for i = 1:Base.length(MarketTiming_weight_Source)
                    if Base.isa(MarketTiming_weight_Source[i], Core.String) && (MarketTiming_weight_Source[i] === "+Base.Inf" || MarketTiming_weight_Source[i] === "+Inf" || MarketTiming_weight_Source[i] === "+inf" || MarketTiming_weight_Source[i] === "+Infinity" || MarketTiming_weight_Source[i] === "+infinity" || MarketTiming_weight_Source[i] === "Base.Inf" || MarketTiming_weight_Source[i] === "Inf" || MarketTiming_weight_Source[i] === "inf" || MarketTiming_weight_Source[i] === "Infinity" || MarketTiming_weight_Source[i] === "infinity")
                        Base.push!(MarketTiming_weight, +Base.Inf);
                    elseif Base.isa(MarketTiming_weight_Source[i], Core.String) && (MarketTiming_weight_Source[i] === "-Base.Inf" || MarketTiming_weight_Source[i] === "-Inf" || MarketTiming_weight_Source[i] === "-inf" || MarketTiming_weight_Source[i] === "-Infinity" || MarketTiming_weight_Source[i] === "-infinity")
                        Base.push!(MarketTiming_weight, -Base.Inf);
                    else
                        Base.push!(MarketTiming_weight, Core.Float64(MarketTiming_weight_Source[i]));
                    end
                end
            end
        end
        # println("URL query MarketTiming weight :\n", MarketTiming_weight);

        if Base.haskey(request_data_Dict, "training_data_file")
            if (Base.typeof(request_data_Dict["training_data_file"]) <: Core.Array || Base.typeof(request_data_Dict["training_data_file"]) <: Base.Vector) && Base.length(request_data_Dict["training_data_file"]) > 0 && Base.isa(request_data_Dict["training_data_file"][1], Core.String) && Base.length(request_data_Dict["training_data_file"][1]) > 0
                training_data_Source = Base.Dict{Core.String, Core.Any}();
                if Base.Filesystem.ispath(Base.string(request_data_Dict["training_data_file"][1])) && Base.Filesystem.isfile(Base.string(request_data_Dict["training_data_file"][1]))
                    # training_data_Source = JLD.load(Base.string(request_data_Dict["training_data_file"][1]), "training_data");
                    training_data_Source = JLD.load(Base.string(request_data_Dict["training_data_file"][1]), "stepping_data");
                else
                    println("training data file = [ " * Base.string(request_data_Dict["training_data_file"][1]) * " ] unrecognized.");
                end
                # if Base.Filesystem.ispath(Cleaned_K_Line_Daily_file_dir) && Base.Filesystem.isfile(Cleaned_K_Line_Daily_file_dir)
                #     training_data_Source = JLD.load(Cleaned_K_Line_Daily_file_dir, "stepping_data");
                # else
                #     println("training data file = [ " * Base.string(Cleaned_K_Line_Daily_file_dir) * " ] unrecognized.");
                # end
                # println(training_data_Source);
                if Base.isa(training_data_Source, Base.Dict) && Base.length(training_data_Source) > 0
                    if (Base.typeof(ticker_symbol_Array) <: Core.Array || Base.typeof(ticker_symbol_Array) <: Base.Vector) && Base.length(ticker_symbol_Array) > 0
                        if !(Base.@isdefined(training_data) && Base.isa(training_data, Base.Dict))
                            training_data = Base.Dict{Core.String, Core.Any}();
                        end
                        if "all" in ticker_symbol_Array
                            # training_data = training_data_Source;
                            for (key, value) in training_data_Source
                                # println("Key: $key, Value:\n$value");
        ...
        if Base.haskey(request_data_Dict, "training_data")
            if Base.isa(request_data_Dict["training_data"], Base.Dict) && Base.length(request_data_Dict["training_data"]) > 0
                if (Base.typeof(ticker_symbol_Array) <: Core.Array || Base.typeof(ticker_symbol_Array) <: Base.Vector) && Base.length(ticker_symbol_Array) > 0
                    if !(Base.@isdefined(training_data) && Base.isa(training_data, Base.Dict))
                        training_data = Base.Dict{Core.String, Core.Any}();
                    end
                    if "all" in ticker_symbol_Array
                        ...
                        for (key, value) in request_data_Dict["training_data"]
                            # println("Key: $key, Value:\n$value");
                            training_data[key] = value;
                        end
                    end
        ...
        # println("Post training data :\n", training_data);
        if Base.haskey(request_Url_Query_Dict, "training_data_file")
            if Base.isa(request_Url_Query_Dict["training_data_file"], Core.String) && Base.length(request_Url_Query_Dict["training_data_file"]) > 0
                training_data_Source = Base.Dict{Core.String, Core.Any}();
                if Base.Filesystem.ispath(Base.string(request_Url_Query_Dict["training_data_file"])) && Base.Filesystem.isfile(Base.string(request_Url_Query_Dict["training_data_file"]))
                    # code_text_Expr = Base.Meta.parse(request_Url_Query_Dict["training_data_file"]);  # 先使用 Base.Meta.parse() 函數解析字符串為代碼;
                    # training_data_Source = Base.MainInclude.eval(code_text_Expr);  # 然後再使用 Base.MainInclude.eval() 函數執行字符串代碼語句;
                    training_data_Source = JLD.load(Base.string(request_Url_Query_Dict["training_data_file"]), "stepping_data");
                    # training_data_Source = JLD.load(Base.string(request_Url_Query_Dict["training_data_file"]), "training_data");
                else
                    println("training data file = [ " * Base.string(request_Url_Query_Dict["training_data_file"]) * " ] unrecognized.");
                end
                # if Base.Filesystem.ispath(Cleaned_K_Line_Daily_file_dir) && Base.Filesystem.isfile(Cleaned_K_Line_Daily_file_dir)
                #     training_data_Source = JLD.load(Cleaned_K_Line_Daily_file_dir, "stepping_data");
                # else
        ...
        # println("URL query training data :\n", training_data);
        if Base.isa(training_data, Base.Dict) && Base.length(training_data) > 0
            for (key, value) in training_data
                if Base.isa(value, Base.Dict) && Base.length(value) > 0
                    if Base.haskey(value, "date_transaction")
                        if (Base.typeof(value["date_transaction"]) <: Core.Array || Base.typeof(value["date_transaction"]) <: Base.Vector) && Base.length(value["date_transaction"]) > 0
                            for i = 1:Base.length(value["date_transaction"])
                                # println("Index: $i, Value: $(value["date_transaction"][i])");
                                if Base.isa(value["date_transaction"][i], Core.String) && value["date_transaction"][i] !== ""
                                    training_data[key]["date_transaction"][i] = Dates.Date(Base.string(Base.strip(Base.string(Base.replace(Base.string(training_data[key]["date_transaction"][i]), "/" => "-")))));
                                end
                            end
                        end
                    end
        ...
        if Base.haskey(request_data_Dict, "testing_data_file")
            if (Base.typeof(request_data_Dict["testing_data_file"]) <: Core.Array || Base.typeof(request_data_Dict["testing_data_file"]) <: Base.Vector) && Base.length(request_data_Dict["testing_data_file"]) > 0 && Base.isa(request_data_Dict["testing_data_file"][1], Core.String) && Base.length(request_data_Dict["testing_data_file"][1]) > 0
                testing_data_Source = Base.Dict{Core.String, Core.Any}();
                if Base.Filesystem.ispath(Base.string(request_data_Dict["testing_data_file"][1])) && Base.Filesystem.isfile(Base.string(request_data_Dict["testing_data_file"][1]))
                    # testing_data_Source = JLD.load(Base.string(request_data_Dict["testing_data_file"][1]), "testing_data");
                    testing_data_Source = JLD.load(Base.string(request_data_Dict["testing_data_file"][1]), "stepping_data");
                else
                    println("testing data file = [ " * Base.string(request_data_Dict["testing_data_file"][1]) * " ] unrecognized.");
                end
                # if Base.Filesystem.ispath(Cleaned_K_Line_Daily_file_dir) && Base.Filesystem.isfile(Cleaned_K_Line_Daily_file_dir)
                #     testing_data_Source = JLD.load(Cleaned_K_Line_Daily_file_dir, "stepping_data");
                # else
                #     println("testing data file = [ " * Base.string(Cleaned_K_Line_Daily_file_dir) * " ] unrecognized.");
                # end
                # println(testing_data_Source);
                if Base.isa(testing_data_Source, Base.Dict) && Base.length(testing_data_Source) > 0
                    if (Base.typeof(ticker_symbol_Array) <: Core.Array || Base.typeof(ticker_symbol_Array) <: Base.Vector) && Base.length(ticker_symbol_Array) > 0
                        if !(Base.@isdefined(testing_data) && Base.isa(testing_data, Base.Dict))
                            testing_data = Base.Dict{Core.String, Core.Any}();
                        end
                        if "all" in ticker_symbol_Array
                            # testing_data = testing_data_Source;
                            for (key, value) in testing_data_Source
                                # println("Key: $key, Value:\n$value");
        ...
        if Base.haskey(request_data_Dict, "testing_data")
            if Base.isa(request_data_Dict["testing_data"], Base.Dict) && Base.length(request_data_Dict["testing_data"]) > 0
                if (Base.typeof(ticker_symbol_Array) <: Core.Array || Base.typeof(ticker_symbol_Array) <: Base.Vector) && Base.length(ticker_symbol_Array) > 0
                    if !(Base.@isdefined(testing_data) && Base.isa(testing_data, Base.Dict))
                        testing_data = Base.Dict{Core.String, Core.Any}();
                    end
                    if "all" in ticker_symbol_Array
                        ...
                        for (key, value) in request_data_Dict["testing_data"]
                            # println("Key: $key, Value:\n$value");
                            testing_data[key] = value;
                        end
                    end
        ...
        # println("Post testing data :\n", testing_data);
        if Base.haskey(request_Url_Query_Dict, "testing_data_file")
            if Base.isa(request_Url_Query_Dict["testing_data_file"], Core.String) && Base.length(request_Url_Query_Dict["testing_data_file"]) > 0
                testing_data_Source = Base.Dict{Core.String, Core.Any}();
                if Base.Filesystem.ispath(Base.string(request_Url_Query_Dict["testing_data_file"])) && Base.Filesystem.isfile(Base.string(request_Url_Query_Dict["testing_data_file"]))
                    # code_text_Expr = Base.Meta.parse(request_Url_Query_Dict["testing_data_file"]);  # 先使用 Base.Meta.parse() 函數解析字符串為代碼;
                    # testing_data_Source = Base.MainInclude.eval(code_text_Expr);  # 然後再使用 Base.MainInclude.eval() 函數執行字符串代碼語句;
                    testing_data_Source = JLD.load(Base.string(request_Url_Query_Dict["testing_data_file"]), "stepping_data");
                    # testing_data_Source = JLD.load(Base.string(request_Url_Query_Dict["testing_data_file"]), "testing_data");
                else
                    println("testing data file = [ " * Base.string(request_Url_Query_Dict["testing_data_file"]) * " ] unrecognized.");
                end
                # if Base.Filesystem.ispath(Cleaned_K_Line_Daily_file_dir) && Base.Filesystem.isfile(Cleaned_K_Line_Daily_file_dir)
                #     testing_data_Source = JLD.load(Cleaned_K_Line_Daily_file_dir, "stepping_data");
                # else
        ...
        # println("URL query testing data :\n", testing_data);
                    ...
                    if Base.haskey(value, "date_transaction")
                        if (Base.typeof(value["date_transaction"]) <: Core.Array || Base.typeof(value["date_transaction"]) <: Base.Vector) && Base.length(value["date_transaction"]) > 0
                            for i = 1:Base.length(value["date_transaction"])
                                # println("Index: $i, Value: $(value["date_transaction"][i])");
                                if Base.isa(value["date_transaction"][i], Core.String) && value["date_transaction"][i] !== ""
                                    testing_data[key]["date_transaction"][i] = Dates.Date(Base.string(Base.strip(Base.string(Base.replace(Base.string(testing_data[key]["date_transaction"][i]), "/" => "-")))));
                                end
                            end
                        end
                    end
        ...
        if Base.haskey(request_data_Dict, "stepping_data_file")
            if (Base.typeof(request_data_Dict["stepping_data_file"]) <: Core.Array || Base.typeof(request_data_Dict["stepping_data_file"]) <: Base.Vector) && Base.length(request_data_Dict["stepping_data_file"]) > 0 && Base.isa(request_data_Dict["stepping_data_file"][1], Core.String) && Base.length(request_data_Dict["stepping_data_file"][1]) > 0
                stepping_data_Source = Base.Dict{Core.String, Core.Any}();
                if Base.Filesystem.ispath(Base.string(request_data_Dict["stepping_data_file"][1])) && Base.Filesystem.isfile(Base.string(request_data_Dict["stepping_data_file"][1]))
                    stepping_data_Source = JLD.load(Base.string(request_data_Dict["stepping_data_file"][1]), "stepping_data");
                else
                    println("stepping data file = [ " * Base.string(request_data_Dict["stepping_data_file"][1]) * " ] unrecognized.");
                end
                # if Base.Filesystem.ispath(Cleaned_K_Line_Daily_file_dir) && Base.Filesystem.isfile(Cleaned_K_Line_Daily_file_dir)
                #     stepping_data_Source = JLD.load(Cleaned_K_Line_Daily_file_dir, "stepping_data");
                # else
                #     println("stepping data file = [ " * Base.string(Cleaned_K_Line_Daily_file_dir) * " ] unrecognized.");
                # end
                # println(stepping_data_Source);
                if Base.isa(stepping_data_Source, Base.Dict) && Base.length(stepping_data_Source) > 0
                    if (Base.typeof(ticker_symbol_Array) <: Core.Array || Base.typeof(ticker_symbol_Array) <: Base.Vector) && Base.length(ticker_symbol_Array) > 0
                        if !(Base.@isdefined(stepping_data) && Base.isa(stepping_data, Base.Dict))
                            stepping_data = Base.Dict{Core.String, Core.Any}();
                        end
                        if "all" in ticker_symbol_Array
                            # stepping_data = stepping_data_Source;
                            for (key, value) in stepping_data_Source
                                # println("Key: $key, Value:\n$value");
        ...
        if Base.haskey(request_data_Dict, "stepping_data")
            if Base.isa(request_data_Dict["stepping_data"], Base.Dict) && Base.length(request_data_Dict["stepping_data"]) > 0
                if (Base.typeof(ticker_symbol_Array) <: Core.Array || Base.typeof(ticker_symbol_Array) <: Base.Vector) && Base.length(ticker_symbol_Array) > 0
                    if !(Base.@isdefined(stepping_data) && Base.isa(stepping_data, Base.Dict))
                        stepping_data = Base.Dict{Core.String, Core.Any}();
                    end
                    if "all" in ticker_symbol_Array
                        ...
                        for (key, value) in request_data_Dict["stepping_data"]
                            # println("Key: $key, Value:\n$value");
                            stepping_data[key] = value;
                        end
                    end
        ...
        # println("Post stepping data :\n", stepping_data);
        if Base.haskey(request_Url_Query_Dict, "stepping_data_file")
            if Base.isa(request_Url_Query_Dict["stepping_data_file"], Core.String) && Base.length(request_Url_Query_Dict["stepping_data_file"]) > 0
                stepping_data_Source = Base.Dict{Core.String, Core.Any}();
                if Base.Filesystem.ispath(Base.string(request_Url_Query_Dict["stepping_data_file"])) && Base.Filesystem.isfile(Base.string(request_Url_Query_Dict["stepping_data_file"]))
                    # code_text_Expr = Base.Meta.parse(request_Url_Query_Dict["stepping_data_file"]);  # 先使用 Base.Meta.parse() 函數解析字符串為代碼;
                    # stepping_data_Source = Base.MainInclude.eval(code_text_Expr);  # 然後再使用 Base.MainInclude.eval() 函數執行字符串代碼語句;
                    stepping_data_Source = JLD.load(Base.string(request_Url_Query_Dict["stepping_data_file"]), "stepping_data");
                else
                    println("stepping data file = [ " * Base.string(request_Url_Query_Dict["stepping_data_file"]) * " ] unrecognized.");
                end
                # if Base.Filesystem.ispath(Cleaned_K_Line_Daily_file_dir) && Base.Filesystem.isfile(Cleaned_K_Line_Daily_file_dir)
                #     stepping_data_Source = JLD.load(Cleaned_K_Line_Daily_file_dir, "stepping_data");
                # else
        ...
        # println("URL query stepping data :\n", stepping_data);
        if Base.isa(stepping_data, Base.Dict) && Base.length(stepping_data) > 0
            for (key, value) in stepping_data
                if Base.isa(value, Base.Dict) && Base.length(value) > 0
                    if Base.haskey(value, "date_transaction")
                        if (Base.typeof(value["date_transaction"]) <: Core.Array || Base.typeof(value["date_transaction"]) <: Base.Vector) && Base.length(value["date_transaction"]) > 0
                            for i = 1:Base.length(value["date_transaction"])
                                # println("Index: $i, Value: $(value["date_transaction"][i])");
                                if Base.isa(value["date_transaction"][i], Core.String) && value["date_transaction"][i] !== ""
                                    stepping_data[key]["date_transaction"][i] = Dates.Date(Base.string(Base.strip(Base.string(Base.replace(Base.string(stepping_data[key]["date_transaction"][i]), "/" => "-")))));
                                end
                            end
                        end
                    end
        ...
        # println("stepping data :\n", stepping_data);

        # 若 testing_data 數據集爲空，則將其賦值爲等同 training_data 數據集;
        if Base.@isdefined(training_data) && ((!Base.@isdefined(testing_data)) || (Base.isa(testing_data, Base.Dict) && Base.length(testing_data) === 0))
            testing_data = training_data;
        end

        response_body_Dict = Base.Dict{Core.String, Core.Any}();
        # response_body_Dict["Cleaned_K_Line"] = Base.string(Cleaned_K_Line_Daily_file_dir);
        response_body_Dict["trading_direction"] = Base.string(investment_method);
        response_body_Dict["is_Optimize"] = Base.string(is_Optimize ? "true" : "false");
        response_body_Dict["ticker_symbol"] = ticker_symbol_Array;
        response_body_Dict["PickStock_Pdata_0"] = PickStock_Pdata_0;
        # response_body_Dict["PickStock_Plower"] = PickStock_Plower;
        # response_body_Dict["PickStock_Pupper"] = PickStock_Pupper;
        # response_body_Dict["PickStock_weight"] = PickStock_weight;
        response_body_Dict["MarketTiming_Pdata_0"] = MarketTiming_Pdata_0;
        # response_body_Dict["MarketTiming_Plower"] = MarketTiming_Plower;
        # response_body_Dict["MarketTiming_Pupper"] = MarketTiming_Pupper;
        # response_body_Dict["MarketTiming_weight"] = MarketTiming_weight;

        # response_body_Dict["stepping_data"] = Base.Dict{Core.String, Core.Any}();
        # stepping_data = Quantitative_Data_Cleaning.stepping_data;
        # if Base.isa(stepping_data, Base.Dict) && Base.length(stepping_data) > 0
        #     for (key, value) in stepping_data
        #         # println("Key: $key, Value:\n$value");
        #         response_body_Dict["stepping_data"][key] = value;
        #     end
        # end

        response_body_Dict["return_PickStock"] = Base.Dict{Core.String, Core.Any}();
        if is_Optimize === false
            ...
            return_PickStock_fit_model = PickStock_fit_model(
                ...
                PickStock_Pdata_0[1],  # Core.Int64(3),  # P1,  # 觀察收益率歷史向前推的交易日長度;
                PickStock_Pdata_0[2],  # Core.Int64(10),  # P2  # 依據市值高低分組選股的分類數目;
                Base.Dict{Core.String, Core.Any}(key => Base.Dict{Core.String, Core.Any}("Long_Position" => MarketTiming_Pdata_0, "Short_Selling" => MarketTiming_Pdata_0) for (key, value) in training_data),  # 按照擇時規則優化之後的參數字典;  # ::Base.Dict{Core.String, Core.Any} = Base.Dict{Core.String, Core.Any}(),
                MarketTiming,
            ...
            if investment_method === "Long_Position_and_Short_Selling"
                response_body_Dict["return_PickStock"]["Coefficient"] = Base.Dict{Core.String, Core.Any}("Long_Position" => PickStock_Pdata_0, "Short_Selling" => PickStock_Pdata_0);
            elseif investment_method === "Long_Position"
                response_body_Dict["return_PickStock"]["Coefficient"] = Base.Dict{Core.String, Core.Any}(Base.string(investment_method) => PickStock_Pdata_0);
            elseif investment_method === "Short_Selling"
                response_body_Dict["return_PickStock"]["Coefficient"] = Base.Dict{Core.String, Core.Any}(Base.string(investment_method) => PickStock_Pdata_0);
            else
            end
            response_body_Dict["return_PickStock"]["PickStock_sort_ticker"] = return_PickStock_fit_model["PickStock_sort"]["ticker_symbol"];  # 依照選股規則排序篩選出的股票代碼字符串存儲數組;
            response_body_Dict["return_PickStock"]["PickStock_sort_score"] = return_PickStock_fit_model["PickStock_sort"]["score"];  # 依照選股規則排序篩選出的股票代碼字符串存儲數組;
            response_body_Dict["return_PickStock"]["y_profit"] = return_PickStock_fit_model["y_profit"];  # 每兩次對衝交易利潤 × 權重，加權纍加總計;
            response_body_Dict["return_PickStock"]["y_Long_Position_profit"] = return_PickStock_fit_model["y_Long_Position_profit"];  # 每兩次對衝交易利潤 × 權重，加權纍加總計;
            response_body_Dict["return_PickStock"]["y_Short_Selling_profit"] = return_PickStock_fit_model["y_Short_Selling_profit"];  # 每兩次對衝交易利潤 × 權重，加權纍加總計;
            response_body_Dict["return_PickStock"]["y_loss"] = return_PickStock_fit_model["y_loss"];  # 每兩次對衝交易最大回撤 × 權重，加權取極值總計;
            response_body_Dict["return_PickStock"]["y_Long_Position_loss"] = return_PickStock_fit_model["y_Long_Position_loss"];  # 每兩次對衝交易最大回撤 × 權重，加權取極值總計;
            response_body_Dict["return_PickStock"]["y_Short_Selling_loss"] = return_PickStock_fit_model["y_Short_Selling_loss"];  # 每兩次對衝交易最大回撤 × 權重，加權取極值總計;
            response_body_Dict["return_PickStock"]["maximum_drawdown"] = return_PickStock_fit_model["maximum_drawdown"];  # 兩次對衝交易之間的最大回撤值，取極值統計;
            response_body_Dict["return_PickStock"]["maximum_drawdown_Long_Position"] = return_PickStock_fit_model["maximum_drawdown_Long_Position"];  # 兩次對衝交易之間的最大回撤值，取極值統計;
            response_body_Dict["return_PickStock"]["maximum_drawdown_Short_Selling"] = return_PickStock_fit_model["maximum_drawdown_Short_Selling"];  # 兩次對衝交易之間的最大回撤值，取極值統計;
            response_body_Dict["return_PickStock"]["profit_total"] = return_PickStock_fit_model["profit_total"];  # 每兩次對衝交易利潤 × 權重，纍加總計;
            response_body_Dict["return_PickStock"]["Long_Position_profit_total"] = return_PickStock_fit_model["Long_Position_profit_total"];  # 每兩次對衝交易利潤 × 權重，纍加總計;
            response_body_Dict["return_PickStock"]["Short_Selling_profit_total"] = return_PickStock_fit_model["Short_Selling_profit_total"];  # 每兩次對衝交易利潤 × 權重，纍加總計;
            response_body_Dict["return_PickStock"]["profit_Positive"] = return_PickStock_fit_model["profit_Positive"];  # 每兩次對衝交易收益纍加總計;
            response_body_Dict["return_PickStock"]["profit_Negative"] = return_PickStock_fit_model["profit_Negative"];  # 每兩次對衝交易損失纍加總計;
            response_body_Dict["return_PickStock"]["Long_Position_profit_Positive"] = return_PickStock_fit_model["Long_Position_profit_Positive"];  # 每兩次對衝交易收益纍加總計;
            response_body_Dict["return_PickStock"]["Long_Position_profit_Negative"] = return_PickStock_fit_model["Long_Position_profit_Negative"];  # 每兩次對衝交易損失纍加總計;
            response_body_Dict["return_PickStock"]["Short_Selling_profit_Positive"] = return_PickStock_fit_model["Short_Selling_profit_Positive"];  # 每兩次對衝交易收益纍加總計;
            response_body_Dict["return_PickStock"]["Short_Selling_profit_Negative"] = return_PickStock_fit_model["Short_Selling_profit_Negative"];  # 每兩次對衝交易損失纍加總計;
            response_body_Dict["return_PickStock"]["profit_Positive_probability"] = return_PickStock_fit_model["profit_Positive_probability"];  # 每兩次對衝交易正利潤概率;
            response_body_Dict["return_PickStock"]["profit_Negative_probability"] = return_PickStock_fit_model["profit_Negative_probability"];  # 每兩次對衝交易負利潤概率;
            response_body_Dict["return_PickStock"]["Long_Position_profit_Positive_probability"] = return_PickStock_fit_model["Long_Position_profit_Positive_probability"];  # 每兩次對衝交易正利潤概率;
            response_body_Dict["return_PickStock"]["Long_Position_profit_Negative_probability"] = return_PickStock_fit_model["Long_Position_profit_Negative_probability"];  # 每兩次對衝交易負利潤概率;
            response_body_Dict["return_PickStock"]["Short_Selling_profit_Positive_probability"] = return_PickStock_fit_model["Short_Selling_profit_Positive_probability"];  # 每兩次對衝交易正利潤概率;
            response_body_Dict["return_PickStock"]["Short_Selling_profit_Negative_probability"] = return_PickStock_fit_model["Short_Selling_profit_Negative_probability"];  # 每兩次對衝交易負利潤概率;
            response_body_Dict["return_PickStock"]["average_price_amplitude_date_transaction"] = return_PickStock_fit_model["average_price_amplitude_date_transaction"];  # 兩兩次對衝交易日成交價振幅平方和，均值;
            response_body_Dict["return_PickStock"]["Long_Position_average_price_amplitude_date_transaction"] = return_PickStock_fit_model["Long_Position_average_price_amplitude_date_transaction"];  # 兩兩次對衝交易日成交價振幅平方和，均值;
            response_body_Dict["return_PickStock"]["Short_Selling_average_price_amplitude_date_transaction"] = return_PickStock_fit_model["Short_Selling_average_price_amplitude_date_transaction"];  # 兩兩次對衝交易日成交價振幅平方和，均值;
            response_body_Dict["return_PickStock"]["average_volume_turnover_date_transaction"] = return_PickStock_fit_model["average_volume_turnover_date_transaction"];  # 兩次對衝交易日成交量（換手率）均值;
            response_body_Dict["return_PickStock"]["Long_Position_average_volume_turnover_date_transaction"] = return_PickStock_fit_model["Long_Position_average_volume_turnover_date_transaction"];  # 兩次對衝交易日成交量（換手率）均值;
            response_body_Dict["return_PickStock"]["Short_Selling_average_volume_turnover_date_transaction"] = return_PickStock_fit_model["Short_Selling_average_volume_turnover_date_transaction"];  # 兩次對衝交易日成交量（換手率）均值;
            response_body_Dict["return_PickStock"]["average_date_transaction_between"] = return_PickStock_fit_model["average_date_transaction_between"];  # 兩次交易間隔日長，均值;
            response_body_Dict["return_PickStock"]["Long_Position_average_date_transaction_between"] = return_PickStock_fit_model["Long_Position_average_date_transaction_between"];  # 兩次對衝交易間隔日長，均值;
            response_body_Dict["return_PickStock"]["Short_Selling_average_date_transaction_between"] = return_PickStock_fit_model["Short_Selling_average_date_transaction_between"];  # 兩次對衝交易間隔日長，均值;
            response_body_Dict["return_PickStock"]["number_PickStock_transaction"] = return_PickStock_fit_model["number_PickStock_transaction"];  # 交易過股票的總隻數;

        elseif is_Optimize === true
            ...
            #             Pupper_weight_MarketTiming_Dict[Base.string(key)] = Base.Dict{Core.String, Core.Any}();
            #         end
            #         MarketTiming_Parameter[Base.string(key)]["Long_Position"] = MarketTiming_Pdata_0;
            #         MarketTiming_Parameter[Base.string(key)]["Short_Selling"] = MarketTiming_Pdata_0;
            #         weight_MarketTiming_Dict[Base.string(key)]["Long_Position"] = MarketTiming_weight;
            #         weight_MarketTiming_Dict[Base.string(key)]["Short_Selling"] = MarketTiming_weight;
            #         Plower_weight_MarketTiming_Dict[Base.string(key)]["Long_Position"] = MarketTiming_Plower;
            #         Plower_weight_MarketTiming_Dict[Base.string(key)]["Short_Selling"] = MarketTiming_Plower;
            #         Pupper_weight_MarketTiming_Dict[Base.string(key)]["Long_Position"] = MarketTiming_Pupper;
            #         Pupper_weight_MarketTiming_Dict[Base.string(key)]["Short_Selling"] = MarketTiming_Pupper;
            #     end
            # end
            ...
            if investment_method === "Long_Position"
                ...
                return_PickStock = PickStock(
                    ...
                    Pdata_0 = PickStock_Pdata_0,  # [Core.Int64(3), Core.Int64(5)],
                    # weight = PickStock_weight,  # Core.Array{Core.Float64, 1}(),
                    Plower = PickStock_Plower,  # [-Base.Inf, -Base.Inf],
                    Pupper = PickStock_Pupper,  # [+Base.Inf, +Base.Inf],
                ...
                if Base.haskey(response_body_Dict["return_PickStock"], "Coefficient") && Base.isa(response_body_Dict["return_PickStock"]["Coefficient"], Base.Dict)
                    response_body_Dict["return_PickStock"]["Coefficient"]["Long_Position"] = return_PickStock["Coefficient"];
                elseif !Base.haskey(response_body_Dict["return_PickStock"], "Coefficient")
                    response_body_Dict["return_PickStock"]["Coefficient"] = Base.Dict{Core.String, Core.Any}();
                    response_body_Dict["return_PickStock"]["Coefficient"]["Long_Position"] = return_PickStock["Coefficient"];
                else
                end
            ...
            elseif investment_method === "Short_Selling"
                ...
                if Base.haskey(response_body_Dict["return_PickStock"], "Coefficient") && Base.isa(response_body_Dict["return_PickStock"]["Coefficient"], Base.Dict)
                    response_body_Dict["return_PickStock"]["Coefficient"]["Short_Selling"] = return_PickStock["Coefficient"];
                elseif !Base.haskey(response_body_Dict["return_PickStock"], "Coefficient")
                    response_body_Dict["return_PickStock"]["Coefficient"] = Base.Dict{Core.String, Core.Any}();
                    response_body_Dict["return_PickStock"]["Coefficient"]["Short_Selling"] = return_PickStock["Coefficient"];
                else
            ...
            elseif investment_method === "Long_Position_and_Short_Selling"
                ...
                response_body_Dict["return_PickStock"]["PickStock_sort_ticker"] = return_PickStock["PickStock_sort"]["ticker_symbol"];  # 依照選股規則排序篩選出的股票代碼字符串存儲數組;
                response_body_Dict["return_PickStock"]["PickStock_sort_score"] = return_PickStock["PickStock_sort"]["score"];  # 依照選股規則排序篩選出的股票代碼字符串存儲數組;
                response_body_Dict["return_PickStock"]["y_profit"] = return_PickStock["testData"]["y_profit"];  # 每兩次對衝交易利潤 × 權重，加權纍加總計;
                response_body_Dict["return_PickStock"]["y_Long_Position_profit"] = return_PickStock["testData"]["y_Long_Position_profit"];  # 每兩次對衝交易利潤 × 權重，加權纍加總計;
                response_body_Dict["return_PickStock"]["y_Short_Selling_profit"] = return_PickStock["testData"]["y_Short_Selling_profit"];  # 每兩次對衝交易利潤 × 權重，加權纍加總計;
                response_body_Dict["return_PickStock"]["y_loss"] = return_PickStock["testData"]["y_loss"];  # 每兩次對衝交易最大回撤 × 權重，加權取極值總計;
                response_body_Dict["return_PickStock"]["y_Long_Position_loss"] = return_PickStock["testData"]["y_Long_Position_loss"];  # 每兩次對衝交易最大回撤 × 權重，加權取極值總計;
                response_body_Dict["return_PickStock"]["y_Short_Selling_loss"] = return_PickStock["testData"]["y_Short_Selling_loss"];  # 每兩次對衝交易最大回撤 × 權重，加權取極值總計;
                response_body_Dict["return_PickStock"]["maximum_drawdown"] = return_PickStock["testData"]["maximum_drawdown"];  # 兩次對衝交易之間的最大回撤值，取極值統計;
                response_body_Dict["return_PickStock"]["maximum_drawdown_Long_Position"] = return_PickStock["testData"]["maximum_drawdown_Long_Position"];  # 兩次對衝交易之間的最大回撤值，取極值統計;
                response_body_Dict["return_PickStock"]["maximum_drawdown_Short_Selling"] = return_PickStock["testData"]["maximum_drawdown_Short_Selling"];  # 兩次對衝交易之間的最大回撤值，取極值統計;
                response_body_Dict["return_PickStock"]["profit_total"] = return_PickStock["testData"]["profit_total"];  # 每兩次對衝交易利潤 × 權重，纍加總計;
                response_body_Dict["return_PickStock"]["Long_Position_profit_total"] = return_PickStock["testData"]["Long_Position_profit_total"];  # 每兩次對衝交易利潤 × 權重，纍加總計;
                response_body_Dict["return_PickStock"]["Short_Selling_profit_total"] = return_PickStock["testData"]["Short_Selling_profit_total"];  # 每兩次對衝交易利潤 × 權重，纍加總計;
                response_body_Dict["return_PickStock"]["profit_Positive"] = return_PickStock["testData"]["profit_Positive"];  # 每兩次對衝交易收益纍加總計;
                response_body_Dict["return_PickStock"]["profit_Negative"] = return_PickStock["testData"]["profit_Negative"];  # 每兩次對衝交易損失纍加總計;
                response_body_Dict["return_PickStock"]["Long_Position_profit_Positive"] = return_PickStock["testData"]["Long_Position_profit_Positive"];  # 每兩次對衝交易收益纍加總計;
                response_body_Dict["return_PickStock"]["Long_Position_profit_Negative"] = return_PickStock["testData"]["Long_Position_profit_Negative"];  # 每兩次對衝交易損失纍加總計;
                response_body_Dict["return_PickStock"]["Short_Selling_profit_Positive"] = return_PickStock["testData"]["Short_Selling_profit_Positive"];  # 每兩次對衝交易收益纍加總計;
                response_body_Dict["return_PickStock"]["Short_Selling_profit_Negative"] = return_PickStock["testData"]["Short_Selling_profit_Negative"];  # 每兩次對衝交易損失纍加總計;
                response_body_Dict["return_PickStock"]["profit_Positive_probability"] = return_PickStock["testData"]["profit_Positive_probability"];  # 每兩次對衝交易正利潤概率;
                response_body_Dict["return_PickStock"]["profit_Negative_probability"] = return_PickStock["testData"]["profit_Negative_probability"];  # 每兩次對衝交易負利潤概率;
                response_body_Dict["return_PickStock"]["Long_Position_profit_Positive_probability"] = return_PickStock["testData"]["Long_Position_profit_Positive_probability"];  # 每兩次對衝交易正利潤概率;
                response_body_Dict["return_PickStock"]["Long_Position_profit_Negative_probability"] = return_PickStock["testData"]["Long_Position_profit_Negative_probability"];  # 每兩次對衝交易負利潤概率;
                response_body_Dict["return_PickStock"]["Short_Selling_profit_Positive_probability"] = return_PickStock["testData"]["Short_Selling_profit_Positive_probability"];  # 每兩次對衝交易正利潤概率;
                response_body_Dict["return_PickStock"]["Short_Selling_profit_Negative_probability"] = return_PickStock["testData"]["Short_Selling_profit_Negative_probability"];  # 每兩次對衝交易負利潤概率;
                response_body_Dict["return_PickStock"]["average_price_amplitude_date_transaction"] = return_PickStock["testData"]["average_price_amplitude_date_transaction"];  # 兩兩次對衝交易日成交價振幅平方和，均值;
                response_body_Dict["return_PickStock"]["Long_Position_average_price_amplitude_date_transaction"] = return_PickStock["testData"]["Long_Position_average_price_amplitude_date_transaction"];  # 兩兩次對衝交易日成交價振幅平方和，均值;
                response_body_Dict["return_PickStock"]["Short_Selling_average_price_amplitude_date_transaction"] = return_PickStock["testData"]["Short_Selling_average_price_amplitude_date_transaction"];  # 兩兩次對衝交易日成交價振幅平方和，均值;
                response_body_Dict["return_PickStock"]["average_volume_turnover_date_transaction"] = return_PickStock["testData"]["average_volume_turnover_date_transaction"];  # 兩次對衝交易日成交量（換手率）均值;
                response_body_Dict["return_PickStock"]["Long_Position_average_volume_turnover_date_transaction"] = return_PickStock["testData"]["Long_Position_average_volume_turnover_date_transaction"];  # 兩次對衝交易日成交量（換手率）均值;
                response_body_Dict["return_PickStock"]["Short_Selling_average_volume_turnover_date_transaction"] = return_PickStock["testData"]["Short_Selling_average_volume_turnover_date_transaction"];  # 兩次對衝交易日成交量（換手率）均值;
                response_body_Dict["return_PickStock"]["average_date_transaction_between"] = return_PickStock["testData"]["average_date_transaction_between"];  # 兩次交易間隔日長，均值;
                response_body_Dict["return_PickStock"]["Long_Position_average_date_transaction_between"] = return_PickStock["testData"]["Long_Position_average_date_transaction_between"];  # 兩次對衝交易間隔日長，均值;
                response_body_Dict["return_PickStock"]["Short_Selling_average_date_transaction_between"] = return_PickStock["testData"]["Short_Selling_average_date_transaction_between"];  # 兩次對衝交易間隔日長，均值;
                response_body_Dict["return_PickStock"]["number_PickStock_transaction"] = return_PickStock["testData"]["number_PickStock_transaction"];  # 交易過股票的總隻數;
            ...
            if investment_method === "Long_Position"
                ...
                return_PickStock_fit_model = PickStock_fit_model(
                    training_data,
                    response_body_Dict["return_PickStock"]["Coefficient"]["Long_Position"][1],  # Core.Int64(3),  # P1,  # 觀察收益率歷史向前推的交易日長度;
                    response_body_Dict["return_PickStock"]["Coefficient"]["Long_Position"][2],  # Core.Int64(10),  # P2  # 依據市值高低分組選股的分類數目;
                    Base.Dict{Core.String, Core.Any}(key => Base.Dict{Core.String, Core.Any}("Long_Position" => MarketTiming_Pdata_0, "Short_Selling" => MarketTiming_Pdata_0) for (key, value) in training_data),  # 按照擇時規則優化之後的參數字典;  # ::Base.Dict{Core.String, Core.Any} = Base.Dict{Core.String, Core.Any}(),
                    MarketTiming,
                    MarketTiming_fit_model,
                    Intuitive_Momentum_KLine,
            ...
            elseif investment_method === "Short_Selling"
                ...
                return_PickStock_fit_model = PickStock_fit_model(
                    training_data,
                    response_body_Dict["return_PickStock"]["Coefficient"]["Short_Selling"][1],  # Core.Int64(3),  # P1,  # 觀察收益率歷史向前推的交易日長度;
                    response_body_Dict["return_PickStock"]["Coefficient"]["Short_Selling"][2],  # Core.Int64(10),  # P2  # 依據市值高低分組選股的分類數目;
                    Base.Dict{Core.String, Core.Any}(key => Base.Dict{Core.String, Core.Any}("Long_Position" => MarketTiming_Pdata_0, "Short_Selling" => MarketTiming_Pdata_0) for (key, value) in training_data),  # 按照擇時規則優化之後的參數字典;  # ::Base.Dict{Core.String, Core.Any} = Base.Dict{Core.String, Core.Any}(),
                    MarketTiming,
                    MarketTiming_fit_model,
                    Intuitive_Momentum_KLine,
        ...
        response_body_Dict["time"] = Base.string(return_file_creat_time);  # Base.Dict("request_POST" => Base.string(request_POST_String), "time" => string(return_file_creat_time));
        # response_body_Dict["Server_Authorization"] = Base.string(key);  # Base.Dict("Server_Authorization" => Base.string(key));
        response_body_Dict["Server_say"] = Base.string("");  # response_body_Dict["return_PickStock"];  # Base.Dict("Server_say" => Base.string(request_POST_String));
        response_body_Dict["error"] = Base.string("");  # Base.Dict("Server_say" => Base.string(request_POST_String));
        # println(response_body_Dict);

        # 將 Julia 數據類型 Core.Nothing 和 Base.Missing 轉換爲 "null" 字符串類型;
        if Base.isa(response_body_Dict, Base.Dict) && Base.haskey(response_body_Dict, "return_PickStock") && Base.isa(response_body_Dict["return_PickStock"], Base.Dict)
            if Base.haskey(response_body_Dict["return_PickStock"], "PickStock_sort_ticker") && (Base.typeof(response_body_Dict["return_PickStock"]["PickStock_sort_ticker"]) <: Core.Array || Base.typeof(response_body_Dict["return_PickStock"]["PickStock_sort_ticker"]) <: Base.Vector)
                for i = 1:Base.length(response_body_Dict["return_PickStock"]["PickStock_sort_ticker"])
                    if !(Base.isa(response_body_Dict["return_PickStock"]["PickStock_sort_ticker"][i], Base.Dict) || (Base.typeof(response_body_Dict["return_PickStock"]["PickStock_sort_ticker"][i]) <: Core.Array || Base.typeof(response_body_Dict["return_PickStock"]["PickStock_sort_ticker"][i]) <: Base.Vector) || Base.isa(response_body_Dict["return_PickStock"]["PickStock_sort_ticker"][i], Core.String)) && (Base.ismissing(response_body_Dict["return_PickStock"]["PickStock_sort_ticker"][i]) || Base.isnothing(response_body_Dict["return_PickStock"]["PickStock_sort_ticker"][i]))
                        response_body_Dict["return_PickStock"]["PickStock_sort_ticker"][i] = "null";
                    end
                    if Base.typeof(response_body_Dict["return_PickStock"]["PickStock_sort_ticker"][i]) <: Core.Array || Base.typeof(response_body_Dict["return_PickStock"]["PickStock_sort_ticker"][i]) <: Base.Vector
                        for j = 1:Base.length(response_body_Dict["return_PickStock"]["PickStock_sort_ticker"][i])
                            if !(Base.isa(response_body_Dict["return_PickStock"]["PickStock_sort_ticker"][i][j], Base.Dict) || (Base.typeof(response_body_Dict["return_PickStock"]["PickStock_sort_ticker"][i][j]) <: Core.Array || Base.typeof(response_body_Dict["return_PickStock"]["PickStock_sort_ticker"][i][j]) <: Base.Vector) || Base.isa(response_body_Dict["return_PickStock"]["PickStock_sort_ticker"][i][j], Core.String)) && (Base.ismissing(response_body_Dict["return_PickStock"]["PickStock_sort_ticker"][i][j]) || Base.isnothing(response_body_Dict["return_PickStock"]["PickStock_sort_ticker"][i][j]))
                                response_body_Dict["return_PickStock"]["PickStock_sort_ticker"][i][j] = "null";
                            end
                        end
                    end
                end
            end
            if Base.haskey(response_body_Dict["return_PickStock"], "PickStock_sort_score") && (Base.typeof(response_body_Dict["return_PickStock"]["PickStock_sort_score"]) <: Core.Array || Base.typeof(response_body_Dict["return_PickStock"]["PickStock_sort_score"]) <: Base.Vector)
                for i = 1:Base.length(response_body_Dict["return_PickStock"]["PickStock_sort_score"])
                    if !(Base.isa(response_body_Dict["return_PickStock"]["PickStock_sort_score"][i], Base.Dict) || (Base.typeof(response_body_Dict["return_PickStock"]["PickStock_sort_score"][i]) <: Core.Array || Base.typeof(response_body_Dict["return_PickStock"]["PickStock_sort_score"][i]) <: Base.Vector) || Base.isa(response_body_Dict["return_PickStock"]["PickStock_sort_score"][i], Core.String)) && (Base.ismissing(response_body_Dict["return_PickStock"]["PickStock_sort_score"][i]) || Base.isnothing(response_body_Dict["return_PickStock"]["PickStock_sort_score"][i]))
                        response_body_Dict["return_PickStock"]["PickStock_sort_score"][i] = "null";
                    end
                    if Base.typeof(response_body_Dict["return_PickStock"]["PickStock_sort_score"][i]) <: Core.Array || Base.typeof(response_body_Dict["return_PickStock"]["PickStock_sort_score"][i]) <: Base.Vector
                        for j = 1:Base.length(response_body_Dict["return_PickStock"]["PickStock_sort_score"][i])
                            if !(Base.isa(response_body_Dict["return_PickStock"]["PickStock_sort_score"][i][j], Base.Dict) || (Base.typeof(response_body_Dict["return_PickStock"]["PickStock_sort_score"][i][j]) <: Core.Array || Base.typeof(response_body_Dict["return_PickStock"]["PickStock_sort_score"][i][j]) <: Base.Vector) || Base.isa(response_body_Dict["return_PickStock"]["PickStock_sort_score"][i][j], Core.String)) && (Base.ismissing(response_body_Dict["return_PickStock"]["PickStock_sort_score"][i][j]) || Base.isnothing(response_body_Dict["return_PickStock"]["PickStock_sort_score"][i][j]))
                                response_body_Dict["return_PickStock"]["PickStock_sort_score"][i][j] = "null";
                            end
                        end
                    end
                end
        ...
        # response_body_Dict = Base.Dict{Core.String, Core.Any}(
        #     "Cleaned_K_Line" => "C:/StatisticalServer/Data/steppingData.jld",
        ...
        #     "MarketTiming_Pupper" => [+Base.Inf, +Base.Inf, +Base.Inf, +Base.Inf],
        #     "MarketTiming_weight" => [],
        #     "return_PickStock" => Base.Dict{Core.String, Core.Any}(
        #         "Coefficient" => [Core.Int64(3), Core.Int64(5)],
        #         "PickStock_sort_ticker" => [Base.string(), Base.string(), Base.string(), ...],  # 依照選股規則排序篩選出的股票代碼字符串存儲數組;
        #         "PickStock_sort_score" => [Core.Float64(), Core.Float64(), Core.Float64(), ...],  # 依照選股規則排序篩選出的股票代碼字符串存儲數組;
        #         "y_profit" => Core.Float64(),  # 每兩次對衝交易利潤 × 權重，加權纍加總計;
        #         "y_Long_Position_profit" => Core.Float64(),  # 每兩次對衝交易利潤 × 權重，加權纍加總計;
        #         "y_Short_Selling_profit" => Core.Float64(),  # 每兩次對衝交易利潤 × 權重，加權纍加總計;
        #         "y_loss" => Core.Float64(),  # 每兩次對衝交易最大回撤 × 權重，加權取極值總計;
        #         "y_Long_Position_loss" => Core.Float64(),  # 每兩次對衝交易最大回撤 × 權重，加權取極值總計;
        #         "y_Short_Selling_loss" => Core.Float64(),  # 每兩次對衝交易最大回撤 × 權重，加權取極值總計;
        #         "maximum_drawdown" => Core.Float64(),  # 兩次對衝交易之間的最大回撤值，取極值統計;
        #         "maximum_drawdown_Long_Position" => Core.Float64(),  # 兩次對衝交易之間的最大回撤值，取極值統計;
        #         "maximum_drawdown_Short_Selling" => Core.Float64(),  # 兩次對衝交易之間的最大回撤值，取極值統計;
        #         "profit_total" => Core.Float64(),  # 每兩次對衝交易利潤 × 權重，纍加總計;
        #         "Long_Position_profit_total" => Core.Float64(),  # 每兩次對衝交易利潤 × 權重，纍加總計;
        #         "Short_Selling_profit_total" => Core.Float64(),  # 每兩次對衝交易利潤 × 權重，纍加總計;
        #         "profit_Positive" => Core.Float64(),  # 每兩次對衝交易收益纍加總計;
        #         "profit_Negative" => Core.Float64(),  # 每兩次對衝交易損失纍加總計;
        #         "Long_Position_profit_Positive" => Core.Float64(),  # 每兩次對衝交易收益纍加總計;
        #         "Long_Position_profit_Negative" => Core.Float64(),  # 每兩次對衝交易損失纍加總計;
        #         "Short_Selling_profit_Positive" => Core.Float64(),  # 每兩次對衝交易收益纍加總計;
        #         "Short_Selling_profit_Negative" => Core.Float64(),  # 每兩次對衝交易損失纍加總計;
        #         "profit_Positive_probability" => Core.Float64(),  # 每兩次對衝交易正利潤概率;
        #         "profit_Negative_probability" => Core.Float64(),  # 每兩次對衝交易負利潤概率;
        #         "Long_Position_profit_Positive_probability" => Core.Float64(),  # 每兩次對衝交易正利潤概率;
        #         "Long_Position_profit_Negative_probability" => Core.Float64(),  # 每兩次對衝交易負利潤概率;
        #         "Short_Selling_profit_Positive_probability" => Core.Float64(),  # 每兩次對衝交易正利潤概率;
        #         "Short_Selling_profit_Negative_probability" => Core.Float64(),  # 每兩次對衝交易負利潤概率;
        #         "average_price_amplitude_date_transaction" => Core.Float64(),  # 兩兩次對衝交易日成交價振幅平方和，均值;
        #         "Long_Position_average_price_amplitude_date_transaction" => Core.Float64(),  # 兩兩次對衝交易日成交價振幅平方和，均值;
        #         "Short_Selling_average_price_amplitude_date_transaction" => Core.Float64(),  # 兩兩次對衝交易日成交價振幅平方和，均值;
        #         "average_volume_turnover_date_transaction" => Core.Int64(),  # 兩次對衝交易日成交量（換手率）均值;
        #         "Long_Position_average_volume_turnover_date_transaction" => Core.Int64(),  # 兩次對衝交易日成交量（換手率）均值;
        #         "Short_Selling_average_volume_turnover_date_transaction" => Core.Int64(),  # 兩次對衝交易日成交量（換手率）均值;
        #         "average_date_transaction_between" => Core.Int64(),  # 兩次交易間隔日長，均值;
        #         "Long_Position_average_date_transaction_between" => Core.Int64(),  # 兩次對衝交易間隔日長，均值;
        #         "Short_Selling_average_date_transaction_between" => Core.Int64(),  # 兩次對衝交易間隔日長，均值;
        #         "number_PickStock_transaction" => Core.Int64()
        #     ),
    ...
    # elseif request_Path === "/SizePosition"
    ...
    # elseif request_Path === "/BackTesting"
    ...
    elseif request_Path === "/Interpolation"
        # 客戶端或瀏覽器請求 url = http://[::1]:10001/Interpolation?Key=username:password&algorithmUser=username&algorithmPass=password&algorithmName=BSpline(Cubic)&algorithmLambda=0&algorithmKei=2&algorithmDi=1&algorithmEith=1
